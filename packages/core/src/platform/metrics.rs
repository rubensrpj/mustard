...
fn emit_metric_inner(cwd: &Path, line: &MetricLine) -> Result<()> {
    if line.event.trim().is_empty() {
        return Err(crate::platform::error::Error::config("metric event name is empty"));
    }
...
mod tests {
    ...
    fn emit_metric_appends_line_to_shard() {
        ...
        let contents = crate::io::fs::read_to_string(&shard).unwrap();
    ...
    fn emit_metric_appends_multiple_lines() {
        ...
        let lines: Vec<_> = crate::io::fs::read_to_string(&shard)
            .unwrap()
            .lines()
            .map(str::to_string)
            .collect();
