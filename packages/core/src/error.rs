...
pub enum Error {
    ...
    /// A [`Check`](crate::model::contract::Check) could not reach a decision
    /// because its [`HookInput`](crate::model::contract::HookInput) was
    /// malformed or missing a field it required.
    #[error("invalid hook input: {0}")]
    InvalidInput(String),

    /// A [`Check`](crate::model::contract::Check) failed for a reason specific
    /// to its own logic.
...
///
/// ```
/// use mustard_core::error::{fail_open, Error, Result};
