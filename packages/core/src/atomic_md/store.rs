...
//!   sibling-tempfile + rename (using the canonical [`crate::fs::write_atomic`]
...
impl MarkdownStore {
    ...
    ///
    /// Uses the canonical [`crate::fs::write_atomic`] seam.
    ///
    /// # Errors
    ///
    ...
    pub fn write_atomic(path: &Path, doc: &MarkdownDoc) -> io::Result<()> {
        ...
        // Delegate to the canonical atomic-write primitive in crate::fs.
