...
//!   resolver lives in [`crate::workspace::workspace_root`]; this guard is
...
impl ClaudePaths {
    ...
    /// [`crate::workspace::workspace_root`] should already have caught the
    /// problem upstream.
    ...
    /// `<root>/.claude/.obsidian/` — Obsidian vault config generated by
    /// `mustard-rt`.
    #[must_use]
    pub fn obsidian_dir(&self) -> PathBuf {
        self.claude_dir().join(".obsidian")
    }
    ...
    /// `<root>/.claude/refs/` — progressive-disclosure refs.
    #[must_use]
    ...
    #[must_use]
    pub fn detect_cache_path(&self) -> PathBuf {
        self.cache_dir().join("detect.json")
    }

    /// `<root>/.claude/.cache/scan-dispatch.json` — scan-dispatch memo.
    #[must_use]
    pub fn scan_dispatch_path(&self) -> PathBuf {
        self.cache_dir().join("scan-dispatch.json")
    }
...
impl SpecPaths {
    ...
    /// `<spec>/adr/` — ADR sidecar directory.
    #[must_use]
    pub fn adr_dir(&self) -> PathBuf {
        self.spec_dir.join("adr")
    }
    ...
    /// `<spec>/.blobs/` — content-addressed blob spill for large event
    /// payloads.
    #[must_use]
    pub fn blobs_dir(&self) -> PathBuf {
        self.spec_dir.join(".blobs")
    }
...
impl WavePaths {
    ...
    /// `<wave>/prompt.md` — the prompt that drove this wave.
    #[must_use]
    pub fn prompt_md_path(&self) -> PathBuf {
        self.wave_dir.join("prompt.md")
    }

    /// `<wave>/warnings.txt` — accumulated warnings emitted during the wave.
    #[must_use]
    pub fn warnings_txt_path(&self) -> PathBuf {
        self.wave_dir.join("warnings.txt")
    }
