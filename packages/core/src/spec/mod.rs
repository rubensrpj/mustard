...
//! `.md` file.
//!
//! This module sits *on top of* [`crate::fs`]: its file-touching wrappers
//! ([`read_state`], [`write_state`]) route through the canonical filesystem
//! seam ([`fs::read_to_string`](crate::fs::read_to_string),
//! [`fs::write_atomic`](crate::fs::write_atomic)) rather than calling
...
pub fn write_state(path: &Path, state: &SpecState) -> std::io::Result<()> {
    let Ok(content) = crate::fs::read_to_string(path) else {
...
mod tests {
    ...
    fn write_state_round_trips_through_disk() {
        ...
        // Body preserved.
        assert!(crate::fs::read_to_string(&path).unwrap().contains("body"));
    }
}
