...
`final` is always `true`; `result` is `approved`/`rejected`; `criterion` is the item's code; `agreed` carries each agreed item, with `met` and what the list asks for when false — the request already carries its format.
