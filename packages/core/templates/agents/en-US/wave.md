...
- Never commit, push or switch branches, and never edit the main repository, the `spec.*` files, the `mustard.json` or its `.claude/`. Before deleting or moving anything in git, prove nothing is lost; without proof, stop and say why.
- Comments follow the project's text language; names, commands and keys stay in English.

## When to stop

Something is missing, a task asks for what the spec does not say, or it does not work as written (a file that does not exist, a contract that does not close): stop and report the problem and the proposed change. Do not decide alone: it only goes ahead when the user clicks "Accept".
...
- `text`: in the project's language, at most 8,000 characters: each changed file in one sentence; for each criterion, the test and its red proof (what was cut and what it said); what you decided outside the request; what is left open, and why.
- `commit`: what the wave did, no spec code, at most 45 characters; the round adds the prefix and refuses over 60.
