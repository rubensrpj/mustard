...
/// Port of `findLastQAResult` — the EventStore-preferred path collapses to a
/// single `events.jsonl` replay here (the JS jsonl fallback).
fn find_last_qa_result(cwd: &str, spec: Option<&str>) -> (bool, Option<String>, usize) {
    let events = JsonlEventStore::for_project(cwd)
        .replay()
        .unwrap_or_default();
...
/// An RFC-3339 / ISO-8601 UTC timestamp string.
...
mod tests {
    ...
    fn write_qa_event(cwd: &Path, spec: &str, overall: &str, criteria: Value) {
        // The harness envelope uses `sessionId` (the `HarnessEvent` field is
        // serde-renamed) — match `qa-run.js`'s real emission shape.
        let event = json!({
            "v": 1,
            "ts": "2026-05-19T00:00:00.000Z",
            "sessionId": "s-test",
            "wave": 0,
            "actor": { "kind": "cli", "id": "qa-run" },
            "event": "qa.result",
            "payload": { "spec": spec, "overall": overall, "criteria": criteria },
        });
        let events = cwd.join(".claude").join(".harness").join("events.jsonl");
