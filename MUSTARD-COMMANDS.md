...
| `page` | Gera uma página no layout do Mustard: `--body <pagina.md> --out <pagina.html>`, com `--title`, `--subtitle` e `--kind` opcionais. `--spec <spec> --owners [<donos.json>]` grava a lista dos itens combinados sem dono (`owners.html`), com o dono proposto e a regra de onde ele veio, para conferir antes de gravar; o arquivo traz o dono que o orquestrador dá. `--spec` sozinho, sem `--owners`, é recusado: a página da spec e a do projeto são templates que leem um banco de dados, e o comando que as refazia saiu. |
