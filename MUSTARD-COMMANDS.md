...
| 7. Pull request | `mustard-rt run pr-open --base <base> --head <branch> --spec <spec>` | Monta o título e o corpo a partir da spec e abre o pull request; o que já existe só tem o corpo reescrito. `--draft` abre como rascunho; `--fill` monta título e corpo pelos commits, para o submódulo sem spec. |
...
O relatório da rodada é um JSON com uma entrada por onda: `{"waves":[{"wave":1,"delivered":"…","files":["…"],"verdict":{…},"replan":"…"}]}`. O `verdict` sai da linha `VERDICT` do agente de revisão, e o `replan`, da linha `DELIVERED` do agente de onda quando o plano não funciona; a mudança só segue com o clique em "Aceitar".
...
| `page` | Gera uma página no layout do Mustard: `--body <pagina.md> --out <pagina.html>`, com `--title`, `--subtitle` e `--kind` opcionais. `--spec <spec>` refaz a página e o `.md` da spec. |
...
| `pr-review` | Sem número, lista os pull requests abertos da base; `--pr <n>` mostra o pedido de revisão, e `--verdict <approved\|rejected> --critical <N>` grava o veredito. |
