...
| 5. Ondas e revisão | `mustard-rt run round --spec <spec>` | Despacha as ondas que podem sair juntas. A onda com item do projeto todo, item sem dono ou lição que casa com ela só sai depois da escolha antes do envio: a rodada devolve em `analysis` esses candidatos ao orquestrador, cada um com o título, e nenhum agente é aberto para isso. Com `--report '<as linhas do fim de cada agente>'`, grava o que cada onda entregou e o veredito da revisão, faz o commit da rodada e despacha a seguinte. |
