...
| 6. Fechamento | `mustard-rt run close --spec <spec>` | Roda cada critério uma vez e pede o agente de teste dedicado, mesmo com uma onda só; só fecha a spec com a linha dele aprovada. `--report '<as linhas do fim de cada agente>'` traz o relatório da última rodada e a linha do agente, no mesmo formato da rodada. |
...
| `page` | Gera uma página no layout do Mustard: `--body <pagina.md> --out <pagina.html>`, com `--title`, `--subtitle` e `--kind` opcionais. `--spec <spec>` refaz a página e o `.md` da spec, e está descontinuado: a resposta avisa que as páginas agora são templates que leem um banco de dados. `--spec <spec> --owners [<donos.json>]` grava a lista dos itens combinados sem dono (`owners.html`), com o dono proposto e a regra de onde ele veio, para conferir antes de gravar; o arquivo traz o dono que o orquestrador dá. |
