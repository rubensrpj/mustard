...
4. Compose `{retry_context}` using Mode=fix-loop format (see `${CLAUDE_PLUGIN_ROOT}/refs/agent-prompt/agent-prompt.md § Retry Modes`). Set K = current loop number (1 or 2; max 2 fix-loops):
   ```
   ## RETRY CONTEXT
   **Mode:** fix-loop ({K}/2)
   ...
   **Review findings (verbatim):**
   {findings_verbatim}
   ```
5. Render the **Minimal Retry Template** from `${CLAUDE_PLUGIN_ROOT}/refs/agent-prompt/agent-prompt.md § Retry Modes` (skips CONTEXT/REFERENCE/ENTITY/SKILLS/WEB VALIDATION/ROLE).
...
3. **Re-dispatch with retry context** — fill `{retry_context}` using Mode=granular format:
   - **Try harness view first:** `mustard-rt run event-projections --view pipeline-state --spec {spec-name}` — use decisions/lessons if available. Fallback: Read `.claude/.agent-memory/_index.json`, find last entry where `agent_type == {failed_agent_type}` and `pipeline == {spec-name}`
   ...
   - Use the **Minimal Retry Template** from `${CLAUDE_PLUGIN_ROOT}/refs/agent-prompt/agent-prompt.md § Retry Modes` (skips CONTEXT/REFERENCE/ENTITY/SKILLS/WEB VALIDATION/ROLE blocks)
