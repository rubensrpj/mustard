...
The dispatch planner (`wave-advance` items carry the field) picks the agent per role via `recommended_subagent_type` — read-only roles run tool-restricted so they physically cannot write; writing roles rely on the per-role contract + the `write_gate` hook. Agents inherit the session model (no routing table).
...
| `{mold_pointer}` | WHICH of those molds govern the files this wave declares — crossed from the wave's `## Arquivos` against each mold's `paths:` | The shelf is the catalogue and is identical across sibling waves; this is the prescription for one wave, which is why it is a separate section further down — folding it into `## SKILLS` would make the shelf vary per wave and cost the prefix siblings share. Empty (section collapses) when nothing matches, when the spec names no files, or for the `patterns` role. |
...
## MOLDS FOR THIS WAVE ({mold_pointer} — the molds the wave's own files fall under; collapses when nothing matches)
