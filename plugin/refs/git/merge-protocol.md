...
The destructive-ops law lives in ONE home — `${CLAUDE_PLUGIN_ROOT}/pipeline-config.md § Destructive-ops Law` (`settings.json permissions.deny` + the `bash_command_gate` safety residue); do not re-list commands here.
