...
```
`tasks` / `files` / `acceptance` are optional (a summary-only entry still scaffolds; no `tasks` emits a stderr WARN). ACs are EARS (`when/then` + a behaviour-asserting `Command:`), never a lone build-green; `analyze-validation` (in `plan-materialize`) WARNs on tautological build/test/grep ACs, AC↔wave/file gaps, and a test-shaped command with no `Expect:` (`test-ac-no-expect`). An AC may add an optional `Expect: <regex>` line (on the same line as its `Command:`, or the next line): opt-in evidence — on exit 0 `qa-run` additionally requires the regex to match the command's combined **stdout+stderr** (a miss fails the AC even though the process exited 0; a malformed regex fails open, so the exit code alone decides). Omit it and the AC keeps the legacy exit-0-passes behaviour. Trace waves to criteria with `satisfies` (AC ids), else `acceptance` lines. Coverage is a gate, not a hint: `wave-scaffold` (via `plan-materialize`) folds the parent `spec.md` `## Acceptance Criteria` ids into the traced set, so **every parent AC MUST be claimed by ≥1 wave** through `satisfies` or an `acceptance` line. Under `MUSTARD_TRACE_GATE_MODE` (default `strict`) an uncovered parent AC makes `wave-scaffold` refuse — exit 2 with a `trace_gaps` array; `warn` prints the gap to stderr and proceeds; `off` silences it. A wave that traces to no AC (an untraced wave) is always a WARN, never blocking. `depends_on` MUST use the `wave-N-role` form (e.g. `["wave-1-backend"]`), never the bare role — an unresolved dep is dropped silently, flattening the DAG to one parallel level. `plan-materialize` writes `wave-plan.md` (table + the localised AC union under `## Acceptance Criteria`, where QA reads) and each `wave-N-{role}/spec.md` (`## Summary` + `## Network` + materialised `## Tasks`/`## Files`); `agent-prompt-render --spec <wave-dir>` reads those back as the agent `## TASK` + `{reference_files}`. Headings render in the project language — do not hand-localise.
