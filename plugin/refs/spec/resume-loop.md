...
The refusal only fires while a round is still owed. Once every impl wave carries a `pipeline.wave.complete`, order has nothing left to decide, so a spec with a contradictory table still gets its review round and its `[]` — nobody has to edit a frozen wave plan just to reach CLOSE.
