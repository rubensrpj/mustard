...
4. **[you]** Present the plan for approval. **Plan mode is the PRIMARY path**: enter plan mode with the wave plan as the plan file body (the full `wave-plan.md` + the wave-spec paths) and call `ExitPlanMode` — the user ACCEPTING the plan mints `<spec>/.approved-by-user` from the harness response (`plan_approval_observer`; you cannot author it), and acceptance ⇒ *approve + implement now* (`implementNow=true`; if the user says in chat to only approve, treat as `implementNow=false`). A rejection keeps plan mode on — adjust in prose and re-present; "Reject decomposition" / "Stop — re-plan" below remain available as chat answers.
   **Fallback (plan mode unavailable):** `AskUserQuestion` — ONE question, primary first. **Attach `wave-plan.md` as the `preview`** of the approval option (never ask approval for a plan the user cannot see); the user's answer mints the same marker. A letter-mode `r` pre-answers it as *approve + implement now*:
...
**Not a wave plan:** print a one-block header (`**{spec}** — PLAN` + `{specSummary}` from `resume-bootstrap`), then **[you]** present for approval — primary: plan mode with the spec body as the plan file, `ExitPlanMode` acceptance = *approve + implement now* (the marker is minted from the harness response); fallback (plan mode unavailable): `AskUserQuestion` — ONE question, **spec body attached as `preview`**: *Approve and implement now* (`true`) / *Approve only — new session* (`false`) / *Adjust-stop*.
...
**[you]** then: (a) optionally record ≤3 architectural decisions — `mustard-rt run emit-event --event decision --spec {spec} --payload "title=…" --payload "rationale=approved at PLAN"`; (b) one `TaskCreate` per agent in the spec; (c) print `[v] ANALYZE [v] PLAN [>] EXECUTE [ ] CLOSE` + a layer line (`Aprovado: camada PRD + camada Plano.`).
...
3. **[you] After each review item returns:** `mustard-rt run review-result --spec {spec} --verdict approved|rejected [--critical N] --subproject {sub}`. This is the "already reviewed" signal — without it the next `wave-advance` re-emits the same item. No commit, no wave-done. REJECTED (any CRITICAL) → `${CLAUDE_PLUGIN_ROOT}/refs/resume/fix-loop-wave.md` (max 2 loops; 2 fails → STOP) before advancing.
...
| REJECTED | Fix Loop (`${CLAUDE_PLUGIN_ROOT}/refs/resume/fix-loop-wave.md`), max 2; 2 fails → STOP |
