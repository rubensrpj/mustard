...
**[you] Model/effort checkpoint — ONCE, at the first EXECUTE entry of this spec** (the §A→§B fall-through with `implementNow=true`, or the first resume that lands in `Execute`; skip it on every later round). The implementer runs as the `mustard:mustard-impl` subagent (isolated in its own worktree) and, declaring no `model`, **inherits the session's model AND effort**, so choosing a different tier for this implementation is the user's own native `/model` / `/effort` (session-level — both reach the inheriting impl agent, and `/effort` is the ONLY way to vary effort: it cannot be scoped per-dispatch). Surface one line — `Implementação herda o modelo/effort da sessão. Trocar? rode /model e/ou /effort agora; senão, "segue".` — and wait for the go before the first dispatch. Do **not** plumb a per-dispatch model or persist a custom field — the docs expose no user-facing per-invocation lever. Review/scan agents are unaffected.
