...
The refusal only fires over a contradiction that still GOVERNS something: a wave ON the loop that has not completed. `cycle` names the loop's own members and nothing else — a wave merely WAITING behind one has a correct `Depends on` cell and dispatches as soon as its dependency completes. Once the cyclic waves carry their `pipeline.wave.complete`, order has nothing left to decide for them, and the spec reaches its review round and its `[]` without anyone editing a frozen wave plan.
