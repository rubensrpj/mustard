...
`/task` is spec-less, so there is no wave plan and no dispatch round — but spec-less is **not** context-less. LOCATE first — **triage by what the scope hands you (→ `${CLAUDE_PLUGIN_ROOT}/refs/locating-code.md`): a LITERAL token (exact symbol, error string, file glob) → `grep`/`glob` it directly and skip the digest; a CONCEPT whose vocabulary may diverge → the scan digest** (the same step `/feature` and `/bugfix` run). Then render each action's prompt with `agent-prompt-render`. **Dispatching without locating sends the agent in blind — the single most common reason a `/task` returns nothing useful.** Render fail-opens on every empty placeholder, so a spec-less invocation is safe.
...
→ See `${CLAUDE_PLUGIN_ROOT}/refs/task/task-prompts.md` for the per-action render invocations.
