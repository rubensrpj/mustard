...
|--------|-------|---------|
| `install` | `/skill install <name-or-path>` | MANUAL — copy the skill folder into `.claude/skills/<name>/` (bundled skills already ship with the plugin; there is no built-in fetch) |
| `create` | `/skill create <name>` | `skill-creator` (see note) |
| `list` | `/skill list` | list `.claude/skills/*/SKILL.md` + read each frontmatter (no dedicated command) |
| `remove` | `/skill remove <name>` | delete `.claude/skills/{name}/` (warn if `source: scan`) |
| `optimize` | `/skill optimize <name>` | `skill-creator` description-optimization (see note) |
| `eval` | `/skill eval <name>` | `skill-creator` eval methodology (see note) |
| `update` | `/skill update` | bundled skills ship with the plugin — update the plugin through its marketplace (or re-run `mustard init`, idempotent); manual skills you copied in are yours to refresh |
...
- `source:` is territorial: `/scan` writes `source: scan` ONLY; `/skill install|create` writes `source: manual` ONLY. Missing `source:` → treat as `manual` (conservative).
