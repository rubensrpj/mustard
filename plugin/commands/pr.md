...
3. Run `mustard-rt run pr-open --base <base> --head <branch> --spec {spec}`. An existing pull request only gets its body rewritten. With submodules, theirs open first and the main one opens with `--draft`.
...
1. Run `mustard-rt run pr-merge --pr <n>`.
