...
Work branch: `/mustard:git push` first, then one PR per repo (submodules first) into each prefix base; do NOT return to base. **While ANY submodule PR is still open the parent opens as a DRAFT**: `rtk gh pr create --base "$BASE" --head <parent-work-branch> --fill --draft --body "Blocked by <sub PR url>"`. GitHub refuses to merge a draft PR, which is what turns "submodules before parent" from a sentence into a block — the order governed only PR OPENING, and on GitHub the two PRs are siblings anyone can merge in either direction. A draft ALSO does not request review from code owners (CODEOWNERS); those requests fire at `gh pr ready`, which runs in `/mustard:git finish` after the bump lands — so expect no reviewers until then. Every submodule PR already merged → open the parent normally (`--fill`, no draft). Bare base `B`: no push → `rtk gh pr create --base <target|flow[B]> --head "$B" --fill`. Existing PR in any repo → print its URL.
