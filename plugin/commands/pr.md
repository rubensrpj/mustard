...
**This step PUBLISHES. It does not judge, and it does not gate.** Every gate this door owns belongs to `merge` — build, tests, QA, review spans, docs. `open` writes a body and creates a pull request, and that is the whole of it. Measured in the field, 2026-08-31: asked for nothing but `open`, a session ran the full suite, found 33 pre-existing red tests, traced them to another unit's spec, discovered the branch was 328 commits behind the base, dry-ran a merge in a throwaway worktree to count conflicts, and came back proposing that merge — with no pull request opened. The operator had asked for a pull request. Everything after the first measurement was work nobody requested, and the one cheap answer that existed (a three-line fix already written down in another spec) was the option offered last.

**A red suite is REPORTED, never investigated here.** The measurement below exists to fill a row of the body, not to decide anything: state the count as it came back, say plainly which failures the diff does not touch, and open the pull request. Whether a pre-existing failure blocks the merge is the reviewer's call and `merge`'s gate — it is not this step's to pre-empt.

**A push refused by the repository's own tooling is REPORTED, not routed around.** A pre-push hook that runs the suite, a protected branch, a missing provider CLI: say what refused and quote it, name the choices in one line each, and stop. Never propose merging an integration base into the unit to make a hook go green — that carries work the operator did not ask for into their branch to fix something that is not theirs. A surgical fix, `--no-verify`, or waiting are all smaller, and the operator chooses.

**The PR body is not yours to write.** It is BUILT from the unit's own event file — the goal becomes the title, the summary you recorded and what each wave delivered become the body — so the reviewer reads the reasoning the unit already holds instead of reconstructing it from a commit list. Your one job is the summary: two or three sentences, recorded as an event. Everything else is `--spec`.
...
- **`merged`** — merged, then settled. The folded `settle` document is `git-settle`'s own report: `repos` carries one entry per repository of the unit, `complete:false` means one is still unsettled, and `alsoMergeable` lists other units awaiting their own merge. Print it verbatim. **Then relay `pendingOpen` to the operator in the same message** — every item of the pending ledger still open after this merge, one line each with its `id` and `title`: agreed work outlives the unit that closed, and the end-of-turn gate refuses a closing message that omits an open item. `pendingClosed` names the item the unit was opened against (`emit-pipeline --pending P-{n}`), closed here with `PR #<n> mergeado` as its reason.
