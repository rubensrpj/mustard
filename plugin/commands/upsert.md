...
1. Run `mustard-rt run upsert`. It writes `.claude/settings.local.json`, `.claude/.gitignore`, `mustard.json`, the session map `mapa-inicio-sessao.md` under `.claude/mustard/` and the three agents under `.claude/agents/mustard/`, all hidden from the project's git. Nothing is committed.
