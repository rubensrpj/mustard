...
1. Run `mustard-rt run upsert`. It writes `.claude/settings.local.json`, `.claude/.gitignore`, `mustard.json`, the session map `session-map.md` under `.claude/mustard/` and the three agents under `.claude/agents/mustard/`, all hidden from the project's git. An older install's map, `mapa-inicio-sessao.md`, leaves the disk, and `mustard.json` then declares the new name. Nothing is committed.
