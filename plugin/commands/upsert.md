...
2. Walk the four lists — `created`, `updated`, `preserved`, `migrated` — and say plainly what each file got. On an update, `.claude/mustard/orchestrator.md` and `.claude/mustard/dispatch.md` are always in `updated`, because every run lays the shipped text down again (e.g. "both router files were rewritten from the shipped rules — if you had edited your copy, that edit is gone"). `preserved` is where the files you own land — the settings file, `.claude/.gitignore`, `mustard.json` — because those are merged, never clobbered.
