...
2. Per `subproject` in the list: `mustard-rt run agent-prompt-render --role patterns --subproject <subproject> --emit ref` (spec-less — the renderer materialises that subproject's slice of the SAME worklist, via the same `collect`, plus the canonical mold format and the delivery contract). Pass the stub to the Task **verbatim**. NEVER assemble this prompt yourself: hand-building it drops the contract the agent needs (`source: scan`, `metadata.generated_by`) and molds come back unusable. If a prompt looks wrong, fix the renderer — a script that builds prompts is always the wrong answer.
3. Dispatch **one agent per subproject** `subagent_type: mustard-patterns` (read-only — returns each mold as a `=== FILE: {moldPath} ===` block, each refusal as a `=== DECLINE: {slug} ===` block), all in ONE message.
4. Relay each `=== FILE ===` block to `mustard-rt run scan-patterns-apply --path <moldPath> --content -` — piping the block on **stdin**, never via a temp file (`--content` defaults to `-`). Relay each `=== DECLINE ===` block to `mustard-rt run scan-patterns-decline --slug <slug> --reason <one line, English>` so the run's final worklist check sees the candidate as settled (the ledger is cleared by the next sweep — no permanent verdicts). NEVER the orchestrator's own Write. Report per subproject: created / preserved (hand-authored) / declined. A `COLLISION` on stderr is a mustard defect, not a preserve — surface it.
