...
1. **Worklist.** `mustard-rt run scan-patterns-list` emits a JSON array `[{subproject, label, slug, moldPath, affix, affixKind, declKind, implements, count, exemplars}]` — every mineable role cluster (**≥3 members**, not under a test/fixture path) attributed to its subproject, with real hand-written exemplars resolved and existing molds already filtered, **capped at 4 per subproject**. Deterministic (byte-stable) and fail-open: a missing/unparseable model → `[]` (exit 0). Empty → skip the pattern enrich silently. The pattern-mold twin of `scan-guards-list` — it REPLACES deriving the worklist by hand from the model (`grain.model.json` is MBs on disk, never a digest you "hold"; hand-slicing it in the orchestrator was the old defect).
2. **Dispatch in parallel.** Group the worklist by `subproject`; dispatch one agent per subproject that has candidates, `subagent_type` `mustard-patterns` (read-only — returns each mold as a demarcated `=== FILE: {moldPath} ===` block). The dispatch prompt lists ONLY that subproject's candidates — each with its `slug`, `label`, `affix` and the worklist `exemplars` paths — plus the canonical mold format and the repo's existing-mold language. Single message, all agents in parallel.
3. **Apply + report.** Relay each returned block to `mustard-rt run scan-patterns-apply --path <moldPath> --content -` (the SKILL.md body on stdin) — NEVER the orchestrator's own Write. The apply is create-only (an existing mold is left untouched), path-shape-guarded and atomic, and — being a `run` command — writes even when the orchestrator runs as a background job (where the Write tool is isolation-gated). Report molds created per subproject; a cluster the agent judged too weak to teach is skipped, not padded.
