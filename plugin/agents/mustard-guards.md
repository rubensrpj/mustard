...
- Read-only: deliver the lines as your raw final message — never write a file; nothing writes them into a `CLAUDE.md` for you.
...
- For npm subprojects, include a Guard requiring a worker cap on test commands (e.g. `jest --maxWorkers=4`, or the runner's equivalent) — uncapped Node test workers (ts-jest especially) can exhaust the machine.
