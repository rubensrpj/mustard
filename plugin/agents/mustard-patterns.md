...
- A worklist entry with `mode: refresh` is a machine-authored mold being regenerated: author it exactly like a create, fresh from the CURRENT exemplars — never read or echo the old mold text (anchoring on stale prose defeats the refresh).
...
- If you refuse a cluster (no teachable shape, exemplars unreadable or generated-only, role already covered by another mold), deliver `=== DECLINE: {slug} ===` followed by a one-line reason and `=== END ===` — the caller records it in `.claude/scan-declined.json` so no future scan re-proposes it. Fewer, sharper molds beat padding.
