...
- **end-to-end RE-MEDIDO pelo fluxo 2-pass (10 casos sialia, juiz Sonnet): 7/10 limpos no arquivo-alvo EXATO** (efetivar→IPayableService, conciliar→ReconciliationService, escalonamento→ReceivableDunningService, extrato→ClientStatementService, parcela→PayableRollingService, cobrar→PaymentGateway/Safe2PayService, assinatura→Safe2PaySubscriptionService). Os 3 restantes são qualidade-de-LABEL/limitação-pontual, NÃO falha do fosso:
  - **baixa** — o alvo `WriteOffAsync` (ReceivableService.cs) ESTAVA nos candidatos (#2), mas o `purpose-judge` mostra só 6 purposes/arquivo e, num serviço grande, o purpose que casou ("Dar baixa por perda/write-off em recebível irrecuperável") caiu fora do cap (alfabético) → juiz cego recusou (`picks:[]`). FIX parcial aplicado (prioriza purposes contendo termo da query antes do cap) MAS insuficiente: "recebível" está em TODO purpose do serviço → não discrimina. **FOLLOW-UP: ranquear por CONTAGEM de termos distintos** (write-off tem dar+baixa+recebível=3 vs "Buscar recebível"=1).
  - **repasse** — o 2-pass surfou a ÁREA PartnerCommission (incl. o MutationService) via `requeryTerms` do digest, mas o juiz bridou a ENTIDADE (`commission`), não a AÇÃO (`markpaid`/`pago`) → pick exato precisa do **termo-de-ação no `requeryTerms`**.
  - **antecipação** era MISLABEL (antecipação=factoring≠forecast); re-rotulado p/ "projeção de recebíveis futuros" → juiz pegou `FinancialDashboardService` (um forecast VÁLIDO, sibling do meu `GetForecastAsync`) → label estreita (há 2 métodos de forecast). O juiz ter recusado a label errada e achado um sibling correto é QUALIDADE, não bug.
- **STATUS Fase 0:** a MÁQUINA do fosso está construída + validada agnóstica (recall IDF + juiz explícito `purpose-judge-render` + auto-heal com `requeryTerms`, tudo wirado no ref DRY `refs/digest-validate.md`; 33 suítes rt+scan verdes, clippy limpo; contrato 100% agnóstico). **NÃO concluída.** Follow-ups p/ cravar o número alto honesto: (1) purpose-judge ranquear purposes por contagem-de-termos (recupera baixa); (2) `requeryTerms` incluir o termo-de-AÇÃO, não só a entidade (recupera repasse); (3) labels rigorosas (1 conceito → 1 alvo inequívoco — antecipação/baixa expuseram ambiguidade). AC do Ponto 1 ainda ABERTOS: MCP `mustard-intel` (NÃO construído), 30 casos/3 projetos, latência P95, repo aberto. NADA committed (dev_rubens); modelo sialia com v3 Sonnet aplicado; léxico restaurado ao original; apply files + labels em job tmp.
