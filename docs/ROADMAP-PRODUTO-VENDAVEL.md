...
- **Fundação pronta e validada** (dev_rubens, NÃO pushed): juiz digest-validate (`centralFound`), léxico+auto-heal, `enrich-purpose` (render→apply, incremental), `purpose-search`, trigger de enrich padrão no `/scan`. Commits `b9fa5e54`, `b8e7ccae`, `64d152c8`. **NOTA (revisado 2026-06-25):** o "purpose-search 10/10" desta linha NÃO foi reproduzível pelo runner determinista — ver bullet "end-to-end RE-MEDIDO" abaixo (o número real é ~7/10 limpo + follow-ups; o 10/10 anterior foi medição ad-hoc/sorte de enrich Haiku).
- **Fase 0 EM ANDAMENTO** (máquina construída, número não cravado — ver STATUS abaixo). Contrato do benchmark escrito em `benchmarks/README.md`.
- **RUNNER IMPLEMENTADO + VALIDADO (2026-06-25, dev_rubens, NÃO committed):** `mustard-rt run recall-bench --labels <ndjson> --model <path>` em `apps/rt/src/commands/recall_bench.rs` (variante `RecallBench` + braço de dispatch + módulo registrados). Reusa `Scan::digest_query` (rank via `.files`) + `Scan::purpose_search` (rank via JSON `.files[].file`), tokeniza com `domain_terms` (idêntico a `feature`/`purpose-search`), emite o JSON do contrato byte-estável. 7 testes unitários da lógica pura (parse ndjson, rank 1-based, recall@k, byte-stable). Fixture sintético público em `benchmarks/fixtures/labels.sample.ndjson`. Validado e2e: name-match com rank não-nulo no próprio repo; **baseline name 0/10 reproduzido PELO RUNNER nos 10 casos reais da sialia** (`nameRecall@1=0.0`, `@5=0.1`).
- **RE-ENRICH NA SIALIA — ENRICH RESOLVIDO COM SONNET (2026-06-25):** re-enriquecida a sialia (5675 métodos) via lotes paralelos. Haiku (2 passes) só deu 3-4/10 — sumário errado/genérico em ~metade (preguiça, NÃO bug do runner/tool: os corpos fatiados estão corretos). **Sonnet (v3, 5672/5675) resolveu a qualidade** — sumários ricos com o verbo de domínio (ReconcileAsync="Concilia manualmente linha de extrato bancário a movimentação"; ClientStatement="Monta extrato do cliente"). **Lição: Haiku NÃO basta para enrich; Sonnet sim.** Modelo sialia ATUALMENTE com v3 Sonnet aplicado (apply files v1/v2/v3 + backup original em job tmp).
- **ARQUITETURA CRISTALIZADA (correção do usuário) — RECALL determinista + PRECISÃO pelo juiz:** o purpose-search NÃO precisa ranquear o alvo em #1; o trabalho dele é **achar os prováveis** (recall — garantir que o alvo SOBREVIVA ao cap), e um **juiz Sonnet** escolhe o certo entre os candidatos. Tudo agnóstico (sialia é só base; o vocabulário de domínio se repete em CADA arquivo/página de negócio, em qualquer sistema — não dá pra escopar por `.tsx`/centralidade).
  - **(feito) IDF no purpose-search** (`apps/scan/src/purpose.rs`, reusa `ranking::idf_x1024` — DRY): rankeia por soma de IDF dos tokens casados, não por contagem-igual. O arquivo que casa o termo RARO/discriminativo sobrevive ao cap em vez de ser despejado pelas centenas de páginas que casam só vocabulário comum. +teste `purpose_search_ranks_rare_term_over_common_pair`. Bench v3+IDF: **8/10 sobrevivem ao cap** (efetivar#2, dar baixa#1, conciliar#10, escalonamento#2, extrato#8, parcela#8, cobrar#3, assinatura#2).
  - **(feito) Passo do juiz EXPLÍCITO** (`apps/rt/src/commands/purpose_judge.rs`, `mustard-rt run purpose-judge-render`): roda o purpose-search, anexa o purpose de cada candidato, e emite o prompt byte-estável que pede ao Sonnet para PICKAR o(s) arquivo(s) que IMPLEMENTAM a ação (vs. a página que só exibe). Antes era leitura solta do orquestrador; agora é passo de 1ª classe (gêmeo do `digest-validate-render`). 3 testes, clippy limpo.
  - **(feito) Auto-heal propõe ponte a partir do purpose-layer + `requeryTerms` especulativo** (`digest_validate.rs` + ref `refs/digest-validate.md`): o juiz agora vê os candidatos do purpose-search como FUNDAMENTAÇÃO (não cego só ao name-match) E, no `centralFound=false`, propõe `requeryTerms` — termos do lado-código (tradução/sinônimo) para RE-CONSULTAR, mesmo SEM ponte fundamentada (busca descartável → chutar é seguro; ≠ `requeryBridges` conservador persistido). Fecha o gap puro de sinônimo: a re-query ressurge o alvo que name+purpose erraram. Contrato 100% agnóstico (zero exemplo de domínio — corrigido após hardcode rejeitado pelo user). +2 testes parse. VALIDADO no repasse: juiz propôs `requeryTerms:["commission","partner-commission","PartnerCommission"]` → re-query do digest surfou `IPartnerCommissionMutationService` (a área do MarkPaid/repasse).
  - **CUSTO de token (confirmado):** conceito ENCONTRADO = 1 pass de juiz (~37k tok); MISS = ~2 passes (~70k) + re-query determinística (0 IA), teto de 1 rodada. Extra só nos misses, desprezível por sessão.
- **end-to-end RE-MEDIDO pelo fluxo 2-pass (10 casos sialia, juiz Sonnet) — NÚMERO CRAVADO: 9/10 no arquivo-alvo EXATO, 10/10 por código-correto** (commit `9d3a96cd`). Os 7 diretos (efetivar→IPayableService, conciliar→ReconciliationService, escalonamento→ReceivableDunningService, extrato→ClientStatementService, parcela→PayableRollingService, cobrar→PaymentGateway/Safe2PayService, assinatura→Safe2PaySubscriptionService) + os 2 fechados nesta sessão:
  - **baixa → ReceivableService (WriteOffAsync) ✓** — FECHADO: o purpose-judge agora ranqueia os purposes por CONTAGEM de termos da query antes do cap de 6/arquivo (o write-off "Dar baixa por perda em recebível irrecuperável" = 3 termos sobrevive; antes caía no alfabético). Juiz casou incobrável≈irrecuperável (cláusula "meaning, not words").
  - **repasse → IPartnerCommissionMutationService (MarkPaid) ✓** — FECHADO via 2-pass: `requeryTerms` agora LIDERA com o verbo de ação (`pay`,`transfer`) além da entidade → re-query do digest surfou o MutationService (#12, sobrevive) → pass-2 juiz pegou "marca comissão aprovada como paga = repasse".
  - **projeção (ex-antecipação) → FinancialDashboardService** — o juiz achou um forecast VÁLIDO (calcula projeção de recebíveis), sibling do meu `GetForecastAsync`; código-correto pra intenção, só não meu arquivo exato (há 2 métodos de forecast). 9/10-exato / 10/10-código-correto. (antecipação original era mislabel — factoring≠forecast.)
- **STATUS Fase 0:** a MÁQUINA do fosso está CONSTRUÍDA + VALIDADA + COMMITADA (`9d3a96cd`, dev_rubens, NÃO pushed): recall IDF + juiz explícito `purpose-judge-render` + auto-heal `requeryTerms` (lidera ação) + purpose-judge por contagem-de-termos; tudo wirado no ref DRY `refs/digest-validate.md`; agnóstico (zero domínio hardcoded); 3284 testes rt+scan verdes, clippy limpo. **Número do fosso cravado: 0/10 name-match → 9-10/10 recall+juiz.** AINDA ABERTO p/ Fase 0 "vendável" completa: MCP `mustard-intel` (NÃO construído), conjunto 30 casos/3 projetos, latência P95, número em REPO ABERTO (métrica-norte pública). Modelo sialia com v3 Sonnet aplicado (não versionado); léxico restaurado ao original; apply files + labels em job tmp.
- **VALIDAÇÃO INDEPENDENTE (2026-06-25, sessão de contexto-completo, gates rodados por mim):** name **0/10** + purpose **9/10 sobrevive ao cap** (recall-bench, model Sonnet v3); juiz Sonnet escolhe o IMPLEMENTADOR (`ReconciliationService` sobre a page) num caso **não-primado**; **3284 testes 0-fail**; build/clippy limpos; `purpose_judge.rs` SOLID + contrato emitido agnóstico. **DEFEITO ACHADO+CONSERTADO:** o contrato `requeryTerms` tinha 1 exemplo de domínio ("commission→pay/mark-paid") = o próprio caso `repasse` do benchmark → violava o agnosticismo E primava esse resultado. **Genericizado** (princípio "lidere com o verbo da operação" mantido, exemplo removido). **repasse RE-MEDIDO LIMPO → HOLDS:** juiz sem cola propôs `requeryTerms:[payout,commission,partner-commission]` e o re-query surfou `IPartnerCommissionMutationService` (o GT) — a contaminação era priming REDUNDANTE. Agnosticismo agora VERDADE.
- **FASE 0 — ENTREGUE NESTA SESSÃO (2026-06-25):** **(1) MCP `find_by_intent`** construído+validado+commitado (`dc203939`): 6ª tool do `mustard-mcp`, wrapper fino sobre `Scan::purpose_search`, o produto "intent→código" que agentes plugam. **(2) Benchmark PÚBLICO Medusa** commitado (`705e73dc`, `benchmarks/medusa/`): repo OSS real (medusajs/medusa, módulo order, 294 métodos, código 100% EN), enriquecido com purposes PT (Sonnet cego, ~$1), 12 recall-holes cross-lingual → **name 0/12 → purpose 9/12@1, 10/12@5, 11/12@12.** A métrica-norte do pitch existe e é reproduzível. RESSALVAS: 12 GT concentrados no god-service (spread fraco, v2 usa arquivos por-ação); 1 miss (`estornar`) = resíduo de sinônimo. AINDA ABERTO: 3º projeto + 30 casos (hoje 22/2 projetos), latência P95, pontes de léxico p/ o resíduo de sinônimo.
- **RESOLUÇÃO DOS PENDENTES (2026-06-25, validação honesta):**
  - ✅ **Spread do Medusa RESOLVIDO** (commit `29f6f387`): benchmark v2 em 11 arquivos por-ação DISTINTOS (`benchmarks/medusa/labels-v2-distinct-files.ndjson`). name @5=0.36 → purpose @5=0.91. Mais honesto que o v1: o recall-hole real é o SUBSTANTIVO cross-lingual (devolução/reclamação/troca), name pega só o cognato do verbo.
  - ❌ **Latência FALHA o AC (1224ms vs <300ms) — DÍVIDA DE PRODUTO #1, não fakeada.** Causa: `purpose_search` re-parseia o model (5,9MB) + reconstrói corpus a CADA call; o MCP herda. FIX (real, ~30-60min, NÃO quick-edit): cachear o model parseado no MCP via once_cell + refatorar `purpose::search` p/ aceitar model pré-parseado (separar build-índice de query). É mais importante que um 3º benchmark — um retrieval de 1,2s/query re-lendo 6MB não é production-grade.
  - ✅ **30/3 FECHADO (3 linguagens)** — commit `0acf46f7`: sialia C#/TS (10/10 juiz) + Medusa TS (name 0.36→purpose 0.91@5) + Saleor Python (`benchmarks/saleor/`, name 0→purpose 0.6@5, 0.7@12). 31 casos / 3 projetos / 3 linguagens. **O fosso rende 0.6–1.0** conforme (a) limpeza da arquitetura (services limpos > GraphQL-heavy) e (b) alinhamento do vocabulário da query com a ação do código. (Bug de setup achado+corrigido: filtro `/order/` pegava `graphql/order`, perdia o app `order/`; e2e pegou.)
  - ⏳ **Resíduo de sinônimo = O LIMITE do fosso, agora caracterizado** (Saleor: faturar≠cobrança, quitado≠pago, reabastecer≠repor — verbo do USUÁRIO ≠ verbo natural do CÓDIGO, ambos PT). O cross-lingual o purpose resolve; o sinônimo-PT-interno NÃO → **ponte de léxico** (`.claude/lexicons`, o ladder já tem tier lexicon). Demo pendente. É o único gap conceitual restante do retrieval.
...
**Fases.**
1. **Cobertura do purpose-search** (fechar o resíduo medido). Os ~3/10 que só casam por sinônimo (`assinatura↔subscrição`) precisam de ponte de léxico OU melhor sumário. Ação no enrich: instruir o sumário a usar a **FORMA canônica (infinitivo) do verbo que ele INFERIU do corpo** (escrever "efetivar", não "efetiva") — **NUNCA uma lista de verbos-alvo.** ⚠️ **INVARIANTE DE VALIDADE: o sumarizador é SEMPRE CEGO às queries/labels do benchmark.** Semear os verbos das perguntas no prompt = ensinar o gabarito → 10/10 falso que não generaliza pros outros 5665 métodos. Mais alimentar pontes sinônimo→sinônimo no léxico a partir dos `matchedTerms` confirmados. Meta: ≥9/10 robusto num conjunto rotulado ampliado (30 casos, 3 projetos), **com sumarizador cego**.
