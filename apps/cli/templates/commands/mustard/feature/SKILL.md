...
- **Promote confirmed bridges**: after a successful re-query that followed a `weak`/`none` report, suggest running `mustard-rt run lexicon-suggest` — it lists the confirmed missed→bridged term pairs; nothing is written unless the user accepts one via `--accept <missed>=<bridged>` (project lexicon overlay only).
