...
- **Fail-open on parent existence.** If `<parent>` does not exist in `.claude/spec/<parent>/`, still create the sub-spec and still emit `spec.link`. The sub-spec is usable standalone; only dashboard navigation is degraded.
