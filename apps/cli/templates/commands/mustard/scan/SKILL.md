...
1. **Worklist (deterministic-first).** From the model you already hold (step 1's digest — do NOT re-run it): for each subproject, list its role clusters with **≥3 member files sharing one convention** (same role folder / affix). Discard clusters living under test/fixture paths (`tests/`, `fixtures/`, `__tests__/`, `spec/`) — a mold teaches production convention; a flat `src/` whose files all share one shape IS a valid single cluster. Then Glob `{subproject}/.claude/skills/*-pattern/SKILL.md` and keep only clusters with NO existing mold. Cap at **≤4 new molds per subproject per scan**. Empty → skip silently.
