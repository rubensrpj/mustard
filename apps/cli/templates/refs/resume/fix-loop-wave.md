...
3. **Re-dispatch with retry context** — fill `{retry_context}` using Mode=granular format:
   ...
   - Use the **Minimal Retry Template** from `.claude/refs/agent-prompt/agent-prompt.md § Retry Modes` (skips CONTEXT/REFERENCE/ENTITY/SKILLS/WEB VALIDATION/ROLE blocks)
