...
//! ```
//!
//! When a subproject has a `[scripts]` block in its
//! `.claude/commands/stack.md` (generated by the structural scan), the
//! declared scripts override the build/test defaults. Otherwise the historical
//! defaults probe applies (cargo/pnpm/python).
//!
//! Per-stack timeouts come from env, defaulting to:
...
/// Per-stack default timeouts. Overridable by the matching
...
/// Last-resort defaults probe — stack-aware. Prefers Rust over
...
/// One subproject's verification outcome.
#[derive(Debug, Clone)]
...
impl VerifyResult {
    /// `{ overall, per_subproject, total_duration_ms, timestamp }`.
...
/// Read `<sub>/.claude/commands/stack.md` and pull declared scripts
...
///
/// After discovery, each target's commands are overridden when its
...
fn discover_targets(cwd: &Path) -> Vec<VerifyTarget> {
    ...
    // Let stack.md scripts win over the discovered defaults.
...
/// Run verification across the target list **sequentially**.
///
...
/// Write the standalone HTML report. Uses the aggregated shape.
///
/// `verify-pipeline` is workspace-wide (not per-spec); the cache reorganisation
...
mod tests {
    ...
    fn verify_targets_json_shape_has_required_keys() {
        // The output shape must carry `overall` + `per_subproject`.
        let result = verify_targets(&[VerifyTarget {
            name: "doc".to_string(),
            cwd: std::env::temp_dir(),
            build: None,
    ...
    fn verify_pipeline_dedups_shared_cargo_workspace() {
        // Two targets in the same cargo workspace (root + member) both
