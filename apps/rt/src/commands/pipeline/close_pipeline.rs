...
mod tests {
    ...
    /// `complete-spec` and `close-pipeline` answer alike after the criteria
    /// move: a new AC, an amended AC, and a failing criterion removed from the
    /// spec file. Each AC is matched to its criterion by its id, so the run of
    /// the new or amended AC lands, and a removed criterion comes back.
    #[test]
    fn close_pipeline_and_complete_spec_agree_after_the_criteria_move() {
        use mustard_core::domain::spec_state::SpecState as _;
        for case in ["new", "amended", "removed"] {
            let dir = tempdir().unwrap();
            let project = dir.path();
            anchor(project);
            let slug = format!("move-{case}");
            let spec_dir = seed_spec(project, &slug, "echo ok");
            crate::shared::spec_state::seed_event(project, &slug, "state", json!({ "phase": "running" }));
            let md = spec_dir.join("spec.md");
            let body = std::fs::read_to_string(&md).unwrap();
            match case {
                "new" => {
                    qa_run::run_qa_with_options(project, &slug, QaRunOptions::default());
                    std::fs::write(&md, format!("{body}- [ ] AC-2: new — Command: `exit 3`\n")).unwrap();
                }
                "amended" => {
                    qa_run::run_qa_with_options(project, &slug, QaRunOptions::default());
                    std::fs::write(&md, body.replace("`echo ok`", "`exit 3`")).unwrap();
                }
                _ => {
                    std::fs::write(&md, format!("{body}- [ ] AC-2: red — Command: `exit 3`\n")).unwrap();
                    qa_run::run_qa_with_options(project, &slug, QaRunOptions::default());
                    let log = crate::shared::spec_state::DiskSpecState::new(project).log(&slug).unwrap();
                    let red = log
                        .visible()
                        .iter()
                        .find(|e| e.event_type == "criterion" && e.str_field("label") == Some("AC-2"))
                        .map(|e| e.id)
                        .unwrap();
                    crate::shared::spec_state::seed_event(
                        ...
                        &slug,
                        "remove",
                        json!({ "targets": [red], "reason": "the failing criterion taken out" }),
                    );
                }
            }
            let completes = crate::commands::spec::complete_spec::run_complete(project, &slug).is_ok();
            let report = close(project, &slug, None);
            assert_eq!(completes, report["completed"] == json!(true), "{case}: {report}");
            assert!(!completes, "{case}: a red criterion never closes");
        }
    }
