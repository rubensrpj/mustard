...
/// Build the `{role_block}` — always the minimal `ROLE: {role}` marker.
///
/// It is deliberately NOT suppressed when a rich `{root}/.claude/agents/
/// {name}-impl.md` exists on disk. A file existing does not mean the
/// `{name}-impl` `subagent_type` is registered in the *current* session: Claude
/// Code loads `.claude/agents/` at session start, so an agent generated by the
/// very scan that is running is not yet dispatchable, and the dispatch then
/// falls back to `general-purpose`. Suppressing the role here based on the file
/// alone would strip the agent's only role cue on exactly that fallback — the
/// same file-exists-vs-registered trap the scan dispatch must avoid. The
/// one-line duplication when the rich agent *is* used natively is negligible.
