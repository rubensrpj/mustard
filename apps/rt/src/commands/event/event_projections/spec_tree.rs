...
mod tests {
    ...
    #[test]
    fn a_spec_folder_without_links_is_a_single_node() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join(".claude").join("spec").join("solo")).unwrap();
        let tree = build_spec_tree(&[], dir.path(), "solo");
        assert_eq!(tree["spec"], json!("solo"));
