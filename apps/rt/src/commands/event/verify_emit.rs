...
fn scan(events: &[HarnessEvent], args: &Args, now_ms: i64) -> VerifyOutcome {
    ...
    for ev in events.iter().rev() {
        ...
        let Some(ts_ms) = crate::commands::spec::complete_spec::parse_iso_millis(&ev.ts) else {
            continue;
        };
        if ts_ms < cutoff {
...
mod tests {
    ...
    fn scan_finds_recent_event() {
        ...
        let now = crate::commands::spec::complete_spec::parse_iso_millis("2026-05-19T00:00:05.000Z").unwrap();
    ...
    fn scan_misses_old_event() {
        ...
        let now = crate::commands::spec::complete_spec::parse_iso_millis("2026-05-19T00:10:00.000Z").unwrap();
    ...
    fn scan_respects_payload_filter() {
        ...
        let now = crate::commands::spec::complete_spec::parse_iso_millis("2026-05-19T00:00:02.000Z").unwrap();
        let mut a = args("qa");
