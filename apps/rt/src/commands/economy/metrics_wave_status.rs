...
//! Waves` table (Spec column); if no plan exists, fall back to
...
/// `Spec` column.
fn parse_plan_rows(wave_plan_text: &str) -> Vec<String> {
...
fn build_result(project: &Path, parent: &str) -> Value {
    ...
    let wave_names: Vec<String> = if plan_rows.is_empty() {
        fallback_wave_dirs(&parent_dir)
    } else {
        plan_rows
    };
...
mod tests {
    ...
    #[test]
    fn parse_plan_rows_reads_spec_column() {
        let plan = "\
| Wave | Spec                  | Role    | Status |
|------|-----------------------|---------|--------|
| 1    | [[wave-1-rt-infra]]   | general | draft  |
| 2    | [[wave-2-skill-tpl]]  | general | queued |
";
        let rows = parse_plan_rows(plan);
        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0], "wave-1-rt-infra");
        assert_eq!(rows[1], "wave-2-skill-tpl");
    }
