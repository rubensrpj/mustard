...
fn resolve_endpoint(value: &str) -> Result<Endpoint, String> {
    if is_tag(value) {
        ...
        if crate::commands::spec::complete_spec::parse_iso_millis(&iso).is_none() {
            return Err(format!("git returned unparseable date for \"{value}\": {iso}"));
        }
        return Ok(Endpoint { iso, source: "tag", raw: value.to_string() });
    }
    if crate::commands::spec::complete_spec::parse_iso_millis(value).is_none() {
        return Err(format!(
...
fn aggregate_window(
    ...
    for entry in entries {
        ...
        for line in content.lines() {
            ...
            let Some(ts_ms) = crate::commands::spec::complete_spec::parse_iso_millis(ts) else {
                continue;
            };
...
fn build_compare(
    ...
    let (Some(from_ms), Some(to_ms)) = (
        crate::commands::spec::complete_spec::parse_iso_millis(&from_ep.iso),
        crate::commands::spec::complete_spec::parse_iso_millis(&to_ep.iso),
    ) else {
