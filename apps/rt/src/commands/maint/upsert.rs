...
//! What the OPERATOR owns is merge-only: an existing `.claude/settings.json`,
//! `.claude/.gitignore` or `mustard.json` is preserved, and only what is
//! missing is created or backfilled. The two injectable instruction files —
