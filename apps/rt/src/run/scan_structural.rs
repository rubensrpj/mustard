...
//! [`crate::run::scan::cluster_discovery`] pass produced. The agent receives a
...
fn build_report(repo_root: &Path, subproject: &str) -> StructuralReport {
    ...
    let stack_id = crate::run::scan::detect_stack(&abs).unwrap_or("unknown").to_string();
    let clusters =
        crate::run::scan::cluster_discovery::discover_clusters(&abs, &stack_id, Some(subproject));
