...
- **Nested rows follow the same rule.** `Role`, `Convention`, `Proj`, `SkeletonEntry` each declare only their consumed fields with the same derive set, and every one carries a `///` line saying what the row is.
...
- **Loading.** A private loader composes the path as `root.join(".claude").join("grain.model.json")` and chains `read_to_string(...).ok().and_then(|raw| serde_json::from_str::<Model>(&raw).ok()).unwrap_or_default()` — absent, unreadable and unparseable are one answer: the empty model.
