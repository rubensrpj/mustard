...
- **Early returns, cheapest first.** `observe` is a chain of `let Some(x) = … else { return; }` / `if !… { return; }`, ordered so the cheapest test fails first, each step carrying a comment saying what it establishes. That chain is the universal shape; the numbered `// Fact N —` labels are NOT — repo-wide they appear in exactly three of the seventeen observers, and they are the same three this bullet calls the deliberate exception below: `approval_marker_observer.rs:355,363`, `plan_approval_observer.rs:67,75`, `picker_approval_observer.rs:155,165` (the labels also merge — `// Facts 2 + 3 —`, `// Facts 1 + 2 —` — when one test settles two facts at once). The numbering is load-bearing only there: each of those three `//!` headers spells out an enumerated list of the facts that must ALL hold before an unforgeable approval marker is minted (`plan_approval_observer.rs:15-27` is the clearest), the body labels are the index back into that list, and the header prose then argues about a fact BY ITS NUMBER (`approval_marker_observer.rs:36,70`, `picker_approval_observer.rs:59`). Everywhere else the guards carry plain prose or no comment at all — `rewave_observer`, `wave_complete_observer`, `wikilink_footer_observer`. Number your steps only if your header doc enumerates them; otherwise just say what the guard establishes. Most observers open with `if ctx.trigger != Some(Trigger::X) { return; }` (`rewave_observer`, `wave_start_observer`, `wave_complete_observer`, `tool_result_observer`, `wikilink_footer_observer`, `change_request_log`, `skill_usage_observer`, `metrics_observer`, `session_cleanup_observer`, `spec_hygiene_observer`) and then, when several tools reach the same trigger, the tool-name match (`if !matches!(input.tool_name.as_deref(), Some("Write" | "Edit")) { return; }` in `rewave_observer` and `wikilink_footer_observer`). The three approval doors are the deliberate exception: each is registered in `registry.rs` for exactly ONE `(Trigger, ToolMatch)` pair, so they trust the dispatcher's filter and open with their first fact instead — `picker_approval_observer` puts the two pure string tests over the payload (`user_typed_text`, `approve_and_implement_letter`) first because it sees every prompt, while `approval_marker_observer` and `plan_approval_observer` resolve the root and make `active_spec` fact 1. Re-check the trigger yourself unless your `applies_to` pins exactly one pair.
