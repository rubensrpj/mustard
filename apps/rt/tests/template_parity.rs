...
const RUNTIME_WHITELIST: &[(&str, &str)] = &[
    (
        "ac-amend",
        "the deliberate amendment of a frozen acceptance criterion \
         (commands/spec/ac_amend.rs); it is reached by hand from the refusal \
         `ac-negative-check` prints, and the shipped planning prose that names \
         it lands with the rest of the amendment path — the sibling ratchet \
         below fails this row the moment it does",
    ),
    (
        "ac-negative-check",
