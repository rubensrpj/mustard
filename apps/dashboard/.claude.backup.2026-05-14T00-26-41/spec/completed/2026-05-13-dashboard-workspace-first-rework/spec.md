...
### Checkpoint: 2026-05-13T05:30:00Z
### Lang: pt
...
- NÃO criar novas tabelas/índices SQL — o dashboard abre conexão `SQLITE_OPEN_READ_ONLY` (`db.rs:13`); criação de índices pertence ao scaffold do Mustard.
...
- `src-tauri/src/db.rs` — adicionar 3 fns puras: `aggregate_activity_from_db`, `quality_metrics_from_db`, `knowledge_browse_from_db`. (sem criação de índices — conexão é RO).
...
- [ ] Adicionar fn pura `aggregate_activity_from_db(conn, limit) -> Vec<ActivityGroup>` — agrupar `events` por `(spec, wave, payload.tool/action_kind, bucket = strftime('%s',ts)/300)`; agregar `count`, `min_ts`, `max_ts`, `sum(input_tokens+output_tokens)` via LEFT JOIN `spans` no mesmo `spec`+`wave`, e `files_touched` via `COUNT(DISTINCT json_extract(payload,'$.target.file'))`. Retornar `vec![]` quando `has_phase1_schema()` é false ou query falha graciosamente.
...
- [ ] AC-6: 3 new DB pure fns defined — Command: `node -e "const s=require('fs').readFileSync('src-tauri/src/db.rs','utf8'); const need=['aggregate_activity_from_db','quality_metrics_from_db','knowledge_browse_from_db']; process.exit(need.every(n => s.includes('fn '+n)) ? 0 : 1)"`
...
- **Índices ausentes — fora do escopo do dashboard**: `idx_events_spec_ts` e `idx_spans_spec_phase` não existem; o dashboard abre o DB em `SQLITE_OPEN_READ_ONLY` (`db.rs:13`), então criar índices pertence ao scaffold Mustard que escreve no DB. As 3 fns novas usam `WHERE spec=? ORDER BY ts DESC` e `ORDER BY duration_ms DESC` — performance fica subótima sem índice mas funcional para os volumes atuais. Acompanhar em spec separado se >50k events causar lentidão.
