...
- The union sits immediately above or beside the interface whose field it types (`DoctorCheck.status: DoctorCheckStatus`), in the same module as the wrapper that returns it — that is what all three exemplars do. Read the rule as "never detached from the shape it types", not as "never under `src/lib/types/`": a vocabulary that belongs to a Rust DTO already mirrored there stays with that DTO (`Stage` and `Outcome` at `src/lib/types/specs.ts:11,22`, in the same file as the two status-adjacent fields this mold cites below).
...
- Nullable status-adjacent fields are `| null` with a doc saying what the UI should render for null (`WorkspaceSummary.tokens_saved_today` at `src/lib/types/specs.ts:249-250` — "render '—' not '0'"); status fields themselves are non-optional.
- Where the backend's status is still an open `string` on the wire, the field stays `status: string` with a comment listing the known values and their casing (`SpecWave.status` at `src/lib/types/specs.ts:183-184` — "queued | in_progress | completed | failed"), instead of a union that would lie about exhaustiveness.
