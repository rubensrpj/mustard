...
- Absent data is spelled `| null` explicitly, not an optional `?` — the UI branches on the null, so the field must always be present (`FrictionEntry.source`, `retry_count`, `api_calls`, `prescription`, `updated_at`). `ProjectEntry` has no absent field at all: `path`, `name` and `addedAt` are each required, because the store writes all three when it creates the entry.
