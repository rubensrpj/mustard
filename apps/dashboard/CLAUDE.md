...
5. **Shared memory** — single truth source `.claude/.harness/mustard.db` (SQLite, consolidado pela spec `eliminate-bun` 2026-05-19). `events.jsonl` no mesmo diretório é log legacy e não é mais autoritativo. Pipeline state é derivado integralmente de eventos via `pipeline_state_for_spec` — não existe mais `.pipeline-states/{spec}.json` como fonte de escrita de status (Wave 4, spec `2026-05-19-pipeline-state-from-sqlite`). Projections de conhecimento (`knowledge.json`, `memory/decisions.json`, `memory/lessons.json`) seguem em uso via INSERT direto em SQLite (tabelas `knowledge_patterns`/`memory_decisions`/`memory_lessons` com FTS5). Readers do dashboard consomem via comandos Tauri `dashboard_specs`/`specs_from_db` que derivam status/fase do último evento `pipeline.status`/`pipeline.phase`. Os leitores JS antigos foram removidos — consumir via `SqliteEventStore` em `mustard-core` ou via `mustard-rt run` subcommands.
