...
/// watcher hands it to the events bus, which fans it out per subscriber.
#[derive(Serialize, Clone)]
#[serde(rename_all = "snake_case")]
...
/// channel. Returns zeros when no economy events exist for the project; a
/// panic degrades to a zeroed summary.
...
// push payload, which the events bus serializes per subscriber.
#[derive(Serialize, Clone)]
#[serde(rename_all = "snake_case")]
...
/// Per-spec card folds over the whole workspace; a panic degrades to an empty
...
fn dashboard_spec_waves_planned(
    repo_path: String,
    spec: String,
...
fn workspace_health(repo_path: String) -> spec_views::WorkspaceHealth {
    crate::catch_panic(move || workspace_health_impl(repo_path))
        .unwrap_or_default()
}
