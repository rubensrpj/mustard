...
/// the debouncer callback so tests can drive batches without an open events
/// stream and assert the one-rebuild-one-emission-per-burst contract.
...
pub fn ensure_watching(
    ...
    let mut debouncer = new_debouncer(
        ...
        move |res: DebounceEventResult| {
            ...
            // — the snapshot push below only covers the specs views. The
            // destination changed from `AppHandle::emit` to the server-sent
            // events bus; the throttling above is untouched.
            ...
            if emissions.rebuild_snapshot {
                ...
                // pay the fold itself. A dedicated thread carries the rebuild;
                // it starts from the incremental caches invalidated inside
                // `process_batch` (milliseconds when warm — only the touched
                // shards are re-parsed) and the emit ships the payload ready
                // to render. At most one rebuild + one emission per burst:
                // scheduling is batch-level and throttled in `process_batch`.
                let repo = repo_clone.clone();
                let bus = bus.clone();
                std::thread::spawn(move || {
                    let snapshot = crate::build_specs_snapshot(&repo);
                    bus.emit("dashboard:specs-snapshot", &snapshot);
                });
            }
        },
    )
