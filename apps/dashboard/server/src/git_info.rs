...
pub fn dashboard_git_info(repo_path: String) -> Result<GitInfo, String> {
    // A join error (panic in the closure) degrades to an empty overview, never
    // an Err toast — the failure-tolerant contract.
    let info = crate::catch_panic(move || git_info_impl(&repo_path))
        .unwrap_or_default();
    Ok(info)
}
...
pub fn dashboard_git_log(
    ...
    let commits =
        crate::catch_panic(move || git_log_impl(&repo_path, &git_ref, limit))
            .unwrap_or_default();
    Ok(commits)
}
