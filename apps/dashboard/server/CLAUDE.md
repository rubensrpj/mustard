...
- Comandos são tolerantes a falha: nunca propague erro de IO ausente pra um toast — devolva vazio/zerado. Onde a versão Tauri degradava um `spawn_blocking(..).await` em pânico, use `crate::catch_panic`; o dispatch também absorve pânico, mas o fallback específico mora no comando.
