...
/**
 * Fetch the honest prompt-economy payload every 60s. `repoPath === null`
 * disables the query (e.g. when no workspace is selected).
 *
 * Mirrors the rest of the dashboard's TanStack Query usage: a stable
 * `queryKey` keyed on `repoPath`, `enabled` gating, and a generous interval
 * because the underlying OTEL aggregation moves at minute granularity.
 */
