...
export const ENV_CATALOG: EnvGroup[] = [
  {
    ...
    keys: [
      ...
      },
      {
        ...
        default: 'strict',
        options: ['strict', 'warn', 'off'],
        desc: 'Exige que a aprovação de uma spec venha do usuário: approve-spec só emite o sinal draft→approved quando o estado da spec, no spec.ndjson, está aprovado — e esse estado só é gravado pela testemunha quando você escolhe "Aprovar" na pergunta "Aprovar esta spec?". O modo de plano e a letra digitada no /mustard:spec não aprovam. Impede o orquestrador de se auto-aprovar.',
        valueDocs: {
          strict: 'Bloqueia approve-spec (exit≠0) sem a aprovação do usuário no estado da spec.',
          warn: 'Avisa mas deixa approve-spec seguir sem a aprovação do usuário.',
          off: 'Desabilita a exigência da aprovação do usuário.',
        },
      },
    ],
  },
