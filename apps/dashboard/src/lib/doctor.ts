...
// Components MUST NOT call `call()` directly — they import from here.
//
// Wraps the `doctor_status` backend command that shells out to
...
/// Call the `doctor_status` backend command and normalise the response. The
...
export async function doctorStatus(projectPath: string): Promise<DoctorStatus> {
  const raw = await call<RawDoctorStatus>("doctor_status", {
    projectPath,
  });
  return {
