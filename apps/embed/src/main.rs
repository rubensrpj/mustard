...
struct Module {
    ...
    /// Mirrors `scan::classify::anchor_eligible`: only empty-class modules embed.
    #[serde(default)]
    file_class: String,
    #[serde(default)]
...
/// Faithful local port of `mustard_core::domain::ast::is_test_path` (the
/// dictionary/purpose anchor gate): whole-segment test dirs, stem suffix/prefix
/// conventions, and the camelCase `FooSpec`/`OrderTest` ending. Local because
/// this sidecar deliberately does not depend on the workspace crates.
fn is_test_path(rel: &str) -> bool {
    const TEST_DIR_SEGMENTS: &[&str] = &[
        "test", "tests", "__tests__", "spec", "specs", "testdata", "fixtures", "__mocks__",
    ];
    ...
    for segment in normalised.split('/') {
        if !segment.is_empty() && TEST_DIR_SEGMENTS.iter().any(|d| *d == segment) {
            return true;
        }
    }
    let stem_of = |file: &str| match file.rfind('.') {
        Some(0) | None => file.to_string(),
    ...
    // camelCase word boundary: `FooSpec`/`OrderTest` yes; `latest`/`attest` no.
    TEST_CAMEL_SUFFIXES.iter().any(|suffix| {
        if stem.len() <= suffix.len() || !stem.ends_with(suffix) {
            return false;
        }
        let boundary = stem_orig.len() - suffix.len();
        let prev_ok = stem_orig[..boundary]
            .chars()
            .next_back()
            .is_some_and(|c| c.is_ascii_lowercase() || c.is_ascii_digit());
        let cap_ok = stem_orig[boundary..].chars().next().is_some_and(char::is_uppercase);
        prev_ok && cap_ok
    })
}

/// Hard cap on the text handed to the embedder, in CHARS (~4 chars/token →
/// ~250 tokens). The head of a method carries the retrieval signal; embedding
/// cost grows superlinearly with length, and this cap is what keeps a full-repo
/// index build inside the ≤15-min product budget.
const MAX_EMBED_CHARS: usize = 1000;

fn cap_embed_text(s: &str) -> String {
    match s.char_indices().nth(MAX_EMBED_CHARS) {
...
fn embed_model(tag: &str) -> EmbeddingModel {
    match tag {
        ...
        "base" | "bge" => EmbeddingModel::BGEBaseENV15,
        // Explicit escape hatch to the 3× bigger multilingual model (better
        // quality, ~4-6× slower — blows the ≤15-min full-repo build budget).
        "e5-base" => EmbeddingModel::MultilingualE5Base,
        // DEFAULT — SMALL multilingual: handles a pt-BR query against EN code
        // directly (no translation) and is the only multilingual size whose
        // full-repo build fits the product's ≤15-min budget on CPU.
        _ => EmbeddingModel::MultilingualE5Small,
    }
}

/// intfloat E5 models are trained with asymmetric "query: "/"passage: " input
/// prefixes (the model card: omitting them degrades retrieval). fastembed does
/// NOT add them (its docs leave prefixing to the caller), so we do — for the
/// E5 tags only; every other family takes raw text.
fn is_e5(tag: &str) -> bool {
    matches!(tag, "e5" | "multi" | "e5-base")
}
fn as_passage(tag: &str, body: &str) -> String {
    if is_e5(tag) { format!("passage: {body}") } else { body.to_string() }
}
fn as_query(tag: &str, intent: &str) -> String {
    if is_e5(tag) { format!("query: {intent}") } else { intent.to_string() }
}
...
fn build() -> anyhow::Result<()> {
    ...
    // Corpus gate = the dictionary/purpose anchor gate: hand-written modules
    // only (`anchor_eligible` ⇔ empty file_class), never tests, never build/dep
    // dirs. Generated/vendored code is retrieval noise AND is what blew the
    // full-repo build past the ≤15-min budget.
    let mut candidates: Vec<Candidate> = Vec::new();
    let mut cache: HashMap<String, String> = HashMap::new();
    for module in &model.modules {
        if !module.file_class.is_empty()
            || is_test_path(&module.path)
            || is_non_source_path(&module.path)
        {
            continue;
        }
        for decl in &module.declarations {
            ...
            let body = cap_embed_text(&slice_body(src, decl.line, 55));
    ...
    eprintln!(
        "mustard-embed build: {} methods — reuse {}, embed {} (model {tag})",
        candidates.len(),
        records.len(),
        to_embed.len()
    );
    ...
    if !to_embed.is_empty() {
        ...
        for chunk in to_embed.chunks(CHUNK) {
            let texts: Vec<String> = chunk.iter().map(|&i| as_passage(&tag, &candidates[i].body)).collect();
...
impl Engine {
    ...
    fn search(&mut self, req: &SearchReq) -> anyhow::Result<serde_json::Value> {
        ...
        let ix = indexes
            .get(&req.vectors)
            .ok_or_else(|| anyhow::anyhow!("index not loaded: {}", req.vectors))?;
        let em = embedders
            .get_mut(&tag)
            .ok_or_else(|| anyhow::anyhow!("embedder not initialised: {tag}"))?;
        let qv = em.embed(vec![as_query(&tag, &req.intent)], None)?
...
fn eval() -> anyhow::Result<()> {
    ...
    for label in &labels {
        let qv = em.embed(vec![as_query(&model_tag, &label.query)], None)?
            .into_iter().next().ok_or_else(|| anyhow::anyhow!("empty query embedding"))?;
...
mod tests {
    ...
    #[test]
    fn e5_prefixes_apply_only_to_e5_tags() {
        assert_eq!(as_query("e5", "listar planos"), "query: listar planos");
        assert_eq!(as_passage("multi", "fn x() {}"), "passage: fn x() {}");
        assert_eq!(as_query("base", "listar planos"), "listar planos", "non-E5 stays raw");
        assert_eq!(as_passage("code", "fn x() {}"), "fn x() {}", "non-E5 stays raw");
    }
    ...
    #[test]
    fn test_path_port_matches_core_conventions() {
        // segment convention (whole segment, never substring)
        assert!(is_test_path("app/specs/foo.ts"));
        assert!(is_test_path("pkg/fixtures/data.cs"));
        assert!(!is_test_path("app/attestation/x.rs"), "substring never matches");
        // stem suffix/prefix conventions
        assert!(is_test_path("src/foo-test.ts"));
        assert!(is_test_path("src/foo.spec.tsx"));
        assert!(is_test_path("src/spec_helpers.rb"));
        // camelCase word boundary
        assert!(is_test_path("Backend/OrderTest.cs"));
        assert!(is_test_path("Backend/FooSpec.cs"));
        assert!(!is_test_path("src/latest.rs"), "'latest' is not a Test word");
        assert!(!is_test_path("src/attest.rs"));
        assert!(!is_test_path("Backend/OrderService.cs"));
    }

    #[test]
    fn embed_text_cap_is_char_boundary_safe() {
        let long = "á".repeat(MAX_EMBED_CHARS + 50);
        let capped = cap_embed_text(&long);
        assert_eq!(capped.chars().count(), MAX_EMBED_CHARS, "caps at N CHARS, not bytes");
        assert_eq!(cap_embed_text("short"), "short", "short text passes through");
    }
}
