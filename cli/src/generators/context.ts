...
import type { ProjectInfo, Analysis, CodeSamples, Stack } from '../types.js';
...
export async function generateContext(
  ...
  if (options.useOllama) {
    const ollamaContent = await generateContextWithOllama(
      projectInfo, analysis, codeSamples, options
    );
    if (ollamaContent) {
...
async function generateContextWithOllama(
  projectInfo: ProjectInfo,
  analysis: Analysis,
  codeSamples: CodeSamples,
  options: ContextGeneratorOptions
): Promise<OllamaContextOutput | null> {
  try {
    const isAvailable = await ollama.checkOllamaAvailable();
    if (!isAvailable) {
      if (options.verbose) console.log('  Ollama not available, using templates');
      return null;
    }
    ...
    if (options.verbose) console.log(`  Using Ollama (${model}) for context generation`);

    const prompt = buildOllamaPrompt(projectInfo, analysis, codeSamples);
    return await ollama.generateJSON<OllamaContextOutput>(prompt, { model, timeout: 90000 });
  } catch {
    if (options.verbose) console.log('  Ollama generation failed, falling back to templates');
    return null;
  }
}
...
async function generateContextFromTemplate(
  contextPath: string,
  projectInfo: ProjectInfo,
  analysis: Analysis
): Promise<string[]> {
...
function getBackendStacks(stacks: Stack[]): Stack[] {
  return stacks.filter(s => ['dotnet', 'node', 'python', 'java', 'go', 'rust', 'fastapi', 'django', 'spring'].includes(s.name));
}

function getFrontendStacks(stacks: Stack[]): Stack[] {
  return stacks.filter(s => ['react', 'nextjs', 'vue', 'angular', 'svelte'].includes(s.name));
}

function getDatabaseStacks(stacks: Stack[]): Stack[] {
  return stacks.filter(s => ['drizzle', 'prisma', 'typeorm', 'sequelize'].includes(s.name));
}

function guessSubprojectRole(name: string): string {
  const lower = name.toLowerCase();
  if (lower.includes('backend') || lower.includes('api') || lower.includes('server')) return 'Backend API';
  if (lower.includes('frontend') || lower.includes('web') || lower.includes('client') || lower.includes('ui')) return 'Frontend App';
  if (lower.includes('database') || lower.includes('db') || lower.includes('data')) return 'Database/Migrations';
  if (lower.includes('lib') || lower.includes('shared') || lower.includes('common')) return 'Shared Libraries';
...
function generateArchitectureMd(projectInfo: ProjectInfo, analysis: Analysis): string {
...
| Project | Path | Role |
|---------|------|------|
${projectInfo.structure.subprojects.map(s =>
  `| **${s.name}** | \`${s.path}\` | ${guessSubprojectRole(s.name)} |`
).join('\n')}
`;
  }

  const stackRows = projectInfo.stacks.map(s =>
...
function formatDepsForPrompt(deps: ProjectInfo['dependencies']): string {
  if (!deps || Object.keys(deps).length === 0) return 'No dependencies scanned';
...
function generateDependenciesSection(projectInfo: ProjectInfo): string {
...
function generatePatternsMd(projectInfo: ProjectInfo, analysis: Analysis): string {
  const backend = getBackendStacks(projectInfo.stacks);
  const frontend = getFrontendStacks(projectInfo.stacks);
  const database = getDatabaseStacks(projectInfo.stacks);
...
function generateBackendPatternSection(stack: Stack): string {
...
function generateFrontendPatternSection(stack: Stack): string {
  const v = stack.version ? ` ${stack.version}` : '';
...
function generateDatabasePatternSection(stack: Stack): string {
  const v = stack.version ? ` ${stack.version}` : '';
...
function generateNamingMd(projectInfo: ProjectInfo): string {
...
function generateBackendNamingSection(stack: Stack): string {
  if (stack.name === 'dotnet') {
...
function generateFrontendNamingSection(stack: Stack): string {
  if (stack.name === 'react' || stack.name === 'nextjs') {
...
function generateDatabaseNamingSection(stack: Stack): string {
  if (stack.name === 'drizzle') {
