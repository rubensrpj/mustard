...
import type { SyncOptions, DependenciesCheck, ProjectInfo, Analysis, DiscoveredPatterns } from '../types.js';
...
export async function syncCommand(options: SyncOptions): Promise<void> {
  ...
  if (syncPrompts) console.log(chalk.green('  ✓ Prompts (merge with existing)'));
  if (syncContext) console.log(chalk.green('  ✓ Context files (regenerate)'));
  if (syncRegistry) console.log(chalk.green('  ✓ Entity registry (update)'));
  ...
  if (syncPrompts && promptsUpdated > 0) changes.push(`${promptsUpdated} prompts`);
  if (syncContext) changes.push('context files');
  if (syncRegistry) changes.push('entity registry');
 ...
 */
async function checkDependencies(options: SyncOptions): Promise<DependenciesCheck> {
 ...
 */
async function syncPromptsWithContext(
  claudePath: string,
