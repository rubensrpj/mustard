...
import type { StructureInfo, Subproject, ArchitectureInfo, ScanOptions, ArchitecturePatternConfig } from '../types.js';

/**
 * Architecture patterns to detect
 */
const ARCHITECTURE_PATTERNS: Record<string, ArchitecturePatternConfig> = {
  mvc: {
 ...
 */
export async function scanStructure(projectPath: string, options: ScanOptions = {}): Promise<StructureInfo> {
 ...
 */
async function detectMonorepo(projectPath: string): Promise<boolean> {
  ...
  if (slnFiles.length > 0) {
    ...
    if (csprojFiles.length >= 2) return true;
  }
 ...
 * Detect architecture pattern
 */
async function detectArchitecture(projectPath: string, directories: string[]): Promise<ArchitectureInfo> {
  const allPaths = await glob('**/', {
    cwd: projectPath,
    ignore: ['**/node_modules/**', '**/bin/**', '**/obj/**', '**/.next/**', '**/dist/**']
  });

  const detectedPatterns: ArchitectureInfo[] = [];

  for (const [patternName, config] of Object.entries(ARCHITECTURE_PATTERNS)) {
    if (config.folders) {
      // Check if all required folders exist
      const hasAll = config.folders.every(folder =>
        directories.some(d => d.toLowerCase() === folder.toLowerCase()) ||
    ...
    if (config.patterns) {
      ...
      const matchesAny = config.patterns.some(pattern =>
        allPaths.some(p => pattern.test(p))
      );
 ...
 */
function detectFolderStyle(directories: string[]): string {
