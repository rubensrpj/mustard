...
import type { Stack, Entity, SearchResult, DiscoveredPatterns, CodeSamples, CodeSample, DiscoverOptions, CodeSampleOptions } from '../types.js';
 ...
 */
export async function discoverPatterns(options: DiscoverOptions = {}): Promise<DiscoveredPatterns> {
  const { verbose = false, stacks = [] } = options;

  const results: DiscoveredPatterns = {
    services: [],
    repositories: [],
    endpoints: [],
    components: [],
 ...
 */
export async function discoverEntities(stacks: Stack[] = []): Promise<Entity[]> {
 ...
 */
export async function getCodeSamples(patterns: DiscoveredPatterns, options: CodeSampleOptions = {}): Promise<CodeSamples> {
 ...
 */
function extractSymbolName(result: SearchResult): string | null {
 ...
 */
function extractEntityName(result: SearchResult): string | null {
  if (!result || !result.content) return null;
  ...
  if (drizzleMatch?.[1]) return drizzleMatch[1];

  // Python model
  const pythonMatch = result.content.match(/class\s+(\w+)\s*\(/);
  if (pythonMatch?.[1]) return pythonMatch[1];
 ...
 */
function inferEntityType(result: SearchResult): string {
  if (!result || !result.file_path) return 'unknown';

  const path = result.file_path.toLowerCase();

  if (path.includes('schema') || path.includes('drizzle')) return 'drizzle';
  if (path.includes('entities') || path.includes('entity')) return 'entity';
