...
function verifyDispatch() {
  ...
  for (const sub of state.dispatch) {
    ...
    // `skills` lists the actual subdir names that contain a SKILL.md on disk.
    // This is the source of truth for "skills_generated" in the /scan return —
    // agent self-reports may overcount (e.g. when prior-run skills survived a
    // failed purge, the agent shortcuts and claims them as freshly generated).
    const verdict = { name: sub.name, skillsDir, status: 'unknown', skillsWritten: 0, skills: [], hasNoPatternsMarker: false };
    ...
    try {
      if (!fs.existsSync(skillsDir)) {
        ...
        verdict.skills.sort();
        verdict.skillsWritten = verdict.skills.length;
        verdict.hasNoPatternsMarker = hasMarker;
        if (verdict.skillsWritten > 0) verdict.status = 'skills';
