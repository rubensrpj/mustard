...
| `settle` | The EXIT RITUAL of a delivered unit — run from the WORK BRANCH after its PR merges (on `dev`/`main` it refuses): verifies 100% merged, advances EVERY local base, then prunes worktree + branch. Delegates WHOLE to `mustard-rt run git-settle` |
...
- **settle** — the unit's exit ritual, AFTER its PR merges. From the WORK BRANCH (never a base — bare settle on `dev`/`main` refuses by design): (1) `mustard-rt run git-settle` — verifies 100% merged (ancestry + `gh` for squash; NOT merged → hard stop, nothing touched) and advances every local base (ff-only / ff-safe `fetch base:base`); the answer is `exit-and-rerun` when run inside the unit's own worktree. (2) `ExitWorktree` (the session returns to the main checkout, whose base is now current). (3) `mustard-rt run git-settle --unit <branch>` — finishes: removes the worktree, deletes the local branch, remote delete fail-open. Print each JSON verbatim; `alsoMergeable` lists other delivered units awaiting their own settle.
