...
Law: NO code before the approved spec — `scope_guard` refuses it anyway. Full scope stops at PLAN; only `/spec` unlocks EXECUTE, and urgency never changes scope. Red flags to stop on: "spec after the code works"; "scope says full but feels light"; "no research, I remember where this lives"; "I'll hand-write spec.md"; "the gate blocked me, I'll work around it". Rationale: `docs/TEMPLATE-RATIONALE.md`.
...
- Call ONCE: `mustard-rt run feature --intent "<lapidated terms + the user's content words>"` — PT hits the code's PT, EN its EN, a wrong guess matches nothing (distinct-union OR query, deterministic, no model call).
...
- **Absence radar (`uncovered`):** request concepts with NO representation among the candidates (probe = folded term + its equivalence expansions). These rows are exactly where the pool is BLIND — settle EACH with one targeted Grep/Glob (existence gate) BEFORE planning, and hand the findings to the dispatched agents alongside your selection. Never conclude "doesn't exist" from the pool alone; an empty `uncovered` means every content concept matched SOMETHING, not that the pool is complete.
- Reading: ONE consolidated Task(Explore) when the survivors fit a single subagent; one Explore per subproject ONLY when anchors genuinely span ≥2 subprojects with volume in each (isolates each subproject's auto-injected CLAUDE.md + skills from the parent). Each Explore returns ≤40 lines. Direct parent reads only for a single-subproject feature too small for a subagent — sliced, never whole files.
- Composition/enhancement (adding tabs, mirroring a panel): the `slices` LEAD — a slice names the recurring pattern and carries `exemplarFiles`; go straight to those, treat the selected candidates as the secondary signal. Net-new entity: a sibling's anchors lead instead.
...
3. `mustard-rt run plan-prepare --from-spec .claude/spec/{slug}/spec.md --slice-match-count <sliceMatchCount from the digest>` — the authority for `scope` (+ decompose/waves).
...
Labels for orientation only (the decision is plan-prepare's): light = 1-2 layers, ≤5 files, mirrors a slice | extended-light = matched slice + modifies existing, 6-8 files | full = 3+ layers, net-new, ≥2 slices with ≥2 layers, or >8 files.
...
- On approve: emit `pipeline.stage: Execute` → `exec-rewave-check` (decomposed → use wave-1 spec) → `dependency-precheck` (block on missing externals) → dispatch via `agent-prompt-render --emit ref` — the 2-line stub stdout IS the Task prompt, passed verbatim (the PreToolUse hook expands it; the full prompt never transits your context); all agents of a wave in one message; each with its role's `subagent_type` → per-wave validate + `memory agent` → REVIEW per subproject (`review-result` emit, max 2 fix loops) → QA (`qa-run`: pass → CLOSE; fail → return the failing AC; skip → warn + allow CLOSE; max 3 iterations).
...
- Research via the digest — never read the repo or `grain.model.json` whole. The digest finds where to look; reading the pointed files is yours.
- Read only the anchors (~12); follow an anchor's references for composition questions. Settle existence/duplication by Grep enumeration BEFORE any subagent — sampled reading never proves absence (Full pre-EXECUTE existence gate: `../../../refs/feature/existence-gate.md`).
