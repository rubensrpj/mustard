...
The binary handles: token extraction (PascalCase via regex), `entity-registry.json` confront (Grep, never Read), path existence confront (Glob), scope inference heuristic, slug derivation, and JSON shape assembly (matches the dashboard's `PrdForm`).
