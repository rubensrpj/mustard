...
2. **Dispatch the judge (Haiku).** Pass the rendered prompt VERBATIM as the Task prompt to `subagent_type: general-purpose` with `model: haiku` (the cheap model — Haiku is right here: a bounded mechanical partition over a short list, not quality-sensitive work; the full list of model pins lives in `pipeline-config.md` § Model). The judge replies with ONLY a JSON array `[{label, concepts, anchors}]`.
