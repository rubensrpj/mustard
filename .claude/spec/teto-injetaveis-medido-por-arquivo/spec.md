---
id: spec.teto-injetaveis-medido-por-arquivo
---

# o teto dos injetaveis e medido por arquivo mas o gancho funde varios num texto so
...
**Como a soma virou previsível.** O censo de terreno era o único termo sem limite —
cerca de 45 caracteres por subprojeto, sem teto. Ele passa a listar no máximo 24
linhas e a DIZER quantas ficaram de fora, com o caminho do censo completo: um censo
que para de listar em silêncio se lê como lista completa. Com o único termo variável
limitado, a reserva que a trava por evento guarda para o que o gancho compõe ao lado
dos injetáveis vira um fato, não uma esperança.

## Arquivos

| arquivo | papel |
|---|---|
| `apps/rt/src/commands/orient.rs` | o censo, agora limitado e honesto sobre o corte |
| `apps/cli/tests/template_budget.rs` | a trava que passa a somar por evento |
| `packages/core/src/platform/project_seed.rs` | a migração que casa grafias equivalentes |
| `packages/core/src/platform/i18n.rs` | a linha que anuncia o truncamento |
