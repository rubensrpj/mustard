...
- [x] AC-W4-6: zero imports antigos `@/components/{specs|workspace|economy|knowledge|prd|telemetry|amend|trace}/` em qualquer arquivo `src/**` — Command: `node -e "const {execSync}=require('child_process');try{const r=execSync('rg --no-heading --no-line-number -l \"@/components/(specs|workspace|economy|knowledge|prd|telemetry|amend|trace)/\" apps/dashboard/src',{encoding:'utf8'});if(r.trim()){console.error('legacy imports remain:\\n'+r);process.exit(1)}}catch(e){if(e.status===1){console.log('ok');process.exit(0)}throw e}console.log('ok')"`
...
- [x] AC-W4-8: zero referências a tokens fantasmas `--color-ok` e `--color-accent-mustard` em `apps/dashboard/src/{features,components,pages}/` (escope da wave: features/+components/; pages/ residual em Wave 5/6) — Command: `node -e "const {execSync}=require('child_process');try{const a=execSync('rg --no-heading -l \"--color-ok\\b\" apps/dashboard/src',{encoding:'utf8'});const b=execSync('rg --no-heading -l \"--color-accent-mustard\\b\" apps/dashboard/src',{encoding:'utf8'});if(a.trim()||b.trim()){console.error('phantom tokens remain:\\n--color-ok:\\n'+a+'\\n--color-accent-mustard:\\n'+b);process.exit(1)}}catch(e){if(e.status===1){console.log('ok');process.exit(0)}throw e}console.log('ok')"`
- [x] AC-W4-9: zero `text-red-(400|500|600|700)` ou `bg-red-(400|500|600|700)` Tailwind raw em `apps/dashboard/src/{features,components,pages}/` — Command: `node -e "const {execSync}=require('child_process');try{const r=execSync('rg --no-heading -l \"(text|bg)-red-(400|500|600|700)\\b\" apps/dashboard/src',{encoding:'utf8'});if(r.trim()){console.error('raw red classes remain:\\n'+r);process.exit(1)}}catch(e){if(e.status===1){console.log('ok');process.exit(0)}throw e}console.log('ok')"`
