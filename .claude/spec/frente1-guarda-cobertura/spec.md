...
O harness detecta, ANTES do QA, quando uma onda deixou arquivos prometidos sem tocar, e **por padrão** impede a onda de ser declarada "completa" — reabrindo-a — em vez de empurrar meia-vertical para o QA. A guarda IMPÕE por padrão; a env `MUSTARD_WAVE_COVERAGE_MODE` só existe como saída de emergência para relaxar (`warn`/`off`).
...
- [x] T1 — módulo `wave_coverage.rs`: `check(wave_dir) -> CoverageVerdict{ok,missing}` (prometidos = itens de arquivo do `meta.json#checklist`; falta = `done:false`) + `mode()` de `MUSTARD_WAVE_COVERAGE_MODE` (Off/Warn/Strict, **padrão Strict = impõe**; env só relaxa) + `blocks(verdict,mode)` (política pura: só strict bloqueia).
