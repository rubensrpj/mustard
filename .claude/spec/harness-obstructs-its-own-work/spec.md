...
- **harness artefact** — a file the Mustard runtime itself writes into `.claude/` while working (`feature-digest.json`, a spec's `qa-report.json` and `qa-report.html`). It is neither code nor user state, and it is what makes the harness dirty its own tree between rituals. Narrowed in review: a spec's `qa/report.md` is NOT one of these — it is the unit's own record of which criteria passed, the peer of `review/verdict.md`, and the close commits it (see Decisions).
