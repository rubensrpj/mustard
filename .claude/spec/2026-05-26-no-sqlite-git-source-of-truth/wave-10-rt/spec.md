...
- [x] AC-3D-1: `cargo build -p mustard-rt` passa após migração. Command: `cargo build -p mustard-rt`
- [x] AC-3D-2: Nenhum dos 4 arquivos referencia `SqliteEventStore` / `sqlite_store` / `memory_sqlite`. Command: `bash -c "! git grep -nE 'SqliteEventStore|sqlite_store|memory_sqlite' -- apps/rt/src/hooks/tool_result.rs apps/rt/src/hooks/notification.rs apps/rt/src/hooks/subagent_inject.rs apps/rt/src/hooks/knowledge.rs"`
