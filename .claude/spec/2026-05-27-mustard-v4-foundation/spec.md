...
- [x] AC-A-2: Plano que contém os tokens `fail-open` ou `empurrar pra W…` dispara o Momento 1 (pré-edit) antes de qualquer chamada Edit — Command: `cargo test -p mustard-rt --lib run::gate_regression_check::tests::test_moment1_amber_on_fail_open_plan -- --exact` (W4, verde 2026-05-28 — plan "vou fazer fail-open dessa wave" gera signal `Vocabulário casou` Layer::Semantic, escalonando a verdict Red/Amber via classify_verdict)
...
- [x] AC-A-6: Verdict amarelo do gate PERGUNTA ao usuário (via AskUserQuestion) — não passa em silêncio — Command: `cargo test -p mustard-rt --lib run::gate_regression_check::tests::test_classify_verdict_amber_emits_askuser -- --exact` (W4, verde 2026-05-28 — `amber_askuser_json()` retorna JSON canônico `{verdict:"amber", askUserQuestion:{question, options:[authorize, block]}, signals:[...]}` traduzido por locale; `emit_amber_askuser_json` imprime no stdout pra orquestrador consumir)
...
- [x] AC-A-9: `_context.md` da wave N+1 tem ≤8.000 palavras quando gerado contra spec com 12 waves anteriores — Command: `cargo test -p mustard-rt --lib run::wave_context::tests::test_context_within_8k_words_for_12_wave_spec -- --exact` (W3, verde 2026-05-28 — `build` retorna `Err(ContextTooLong)` quando excede, jamais trunca silenciosamente)
...
- [x] AC-A-15: Spec sem `## Funções tocadas` → fallback usa funções públicas tocadas pelo diff; gate funciona sem panic em fixture de spec legada — Command: `cargo test -p mustard-core --lib spec::touched_functions::tests::ac_a_15_legacy_no_funcoes_fixture_falls_back_via_real_file -- --exact` (W0 fixture + W4 integração, verde 2026-05-28 — fixture `legacy-no-funcoes/spec.md` lida do disco, `parse()` retorna None corretamente honrando fences markdown, closure de fallback invocada exatamente 1×)
...
- ~~**Acessor `VocabularyDoc::layer_terms`** (W5 follow-up #2)~~ — **RESOLVIDO 2026-05-28**. `mustard_core::vocabulary::VocabularyDoc::layer_terms(kind: Layer) -> Vec<&str>` adicionado; `subagent_inject::read_vocab_layers` e `agent_prompt_render::read_vocab_layers_for_inject` migrados pra usar o accessor.
- ~~**Subcomando CLI para span-level eval** (W5 follow-up #3)~~ — **RESOLVIDO 2026-05-28**. `mustard-rt run gate-regression-check --moment 3 --wave-dir <path>` agora consulta o ledger `_review-spans.md` via `check_consolidation` e exita 0/2 sem passar pelo hook `SubagentStop`. Útil pra scripts de close-gate standalone.
...
- ~~**Boundary warnings stale** (W5 follow-up #4)~~ — **RESOLVIDO 2026-05-28**. `post_edit::check_boundaries` agora ranqueia specs Active por `### Checkpoint:` (fallback: prefixo `YYYY-MM-DD` do nome do dir) — em vez de iterar `read_dir` em ordem alfabética. A spec mais recente vence; o warning agora cita a spec correta. Reinstalar o binário (`cargo install --path apps/rt --force`) para o hook ativo pegar o novo resolver. Testes `check_boundaries_picks_newer_active_spec_over_older` + `_warns_when_newer_active_spec_excludes_the_edit`.
...
- ~~**Thresholds via TOML em `[thresholds]`** (W7 follow-up #2, ~60 LOC, opcional)~~ — **RESOLVIDO 2026-05-28**. `VocabularyDoc.thresholds.line_change_threshold` (opcional) + `gate_regression_check::load_line_change_threshold` resolvem o cap em runtime a partir de `.claude/vocab/regression.toml#[thresholds]`; fallback para `LINE_CHANGE_THRESHOLD = 5` quando ausente. `snapshot_signal_with_threshold` escala severidade contra o threshold dinâmico (High quando `line_changes > threshold × 2`).
