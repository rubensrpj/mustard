...
- **Parser do `qa-run` deve tolerar trailing-text após o `\`code\`` do `Command:`** (W8 follow-up #3, ~15 LOC) — `parse_ac_line` em `apps/rt/src/run/qa_run.rs:188-195` consome até o fim da linha; cargo recebe `cargo test … (entregue em W1)` e reclama. Sub-spec tática curta ou mudança inline em qualquer outra wave que toque `qa_run.rs`. *Como aplicar:* capturar só entre o primeiro par de backticks; cobertura via teste regressão.
