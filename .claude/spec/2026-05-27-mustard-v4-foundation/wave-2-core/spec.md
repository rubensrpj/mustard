...
Terceira primitiva de `mustard-core`. Entrega a foto antes/depois — captura o estado de cada função declarada em `## Funções tocadas`, compara dois snapshots e produz `Vec<FunctionDelta>`. Camada 3 do gate de regressão consome esta primitiva no Momento 3 (fechamento da wave). Serialização canônica via `serde_json` (campos ordenados, bytes estáveis — diff reprodutível entre máquinas).
...
- [ ] T2.2: Implementar `regression_check::Snapshot::capture_for_spec` em `packages/core/src/regression_check/snapshot.rs` lendo `## Funções tocadas` (W0) e usando AST (W1.5) pra capturar corpo de função
- [ ] T2.3: Implementar `regression_check::compare_snapshots` e `Snapshot::compare_to` em `packages/core/src/regression_check/compare.rs` produzindo `Vec<FunctionDelta>` (AC-A-4)
- [ ] T2.4: Estender `packages/core/Cargo.toml` adicionando `similar = "2"` e re-exportar `regression_check` em `packages/core/src/lib.rs`
...
- W1.5 (parser AST para extrair corpo de função no `capture_for_spec`)