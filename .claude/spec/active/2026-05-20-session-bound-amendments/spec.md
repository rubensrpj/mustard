...
### Phase: CLOSE
### Scope: full
### Checkpoint: 2026-05-20T11:30:00Z
### Lang: pt
...
- [x] AC-16: 4 Tauri commands (`amend_resolution_rate`, `amend_drift_rate`, `cross_session_amend_count`, `amend_window_duration`) registrados em lib.rs e AmendMetricsCard consome todos — Command: `node -e "const fs=require('fs');const l=fs.readFileSync('apps/dashboard/src-tauri/src/lib.rs','utf8');const c=fs.readFileSync('apps/dashboard/src/components/amend/AmendMetricsCard.tsx','utf8');const cmds=['amend_resolution_rate','amend_drift_rate','cross_session_amend_count','amend_window_duration'];process.exit(cmds.every(k=>l.includes(k)&&c.includes(k))?0:1)"`
...
- [ ] Adicionar tabela `pipeline_amend_window` + índice `(session_id, status)` em `sqlite_schema.sql`
- [ ] Adicionar 5 constantes `EVENT_PIPELINE_AMEND_*` e payload structs serde lenient em `event.rs`
- [ ] Implementar `SqliteEventStore::amend_window_for_session(session_id) -> Result<Option<AmendWindow>>`
- [ ] Implementar `SqliteEventStore::amend_window_pipeline_file_set(spec_id) -> Result<Vec<String>>` (união de `pipeline.task.complete.files_modified`)
- [ ] Test: schema migration roda limpa sobre banco existente (AC-1)
- [ ] Test: projeção retorna janela completa com `pipeline_file_set` correto após 3 task.complete events sintéticos (AC-2, AC-7)
- [ ] `cargo build -p mustard-core && cargo test -p mustard-core`

### rt Agent (Wave 2) — Hook amend_capture + drift detection

- [ ] Criar `apps/rt/src/hooks/amend_capture.rs` implementando Check + Observer
- [ ] No evento `pipeline.complete`: abrir janela (INSERT OR IGNORE) com `pipeline_file_set` derivado + `subprojects` parseados — emit `pipeline.amend_open`
- [ ] PostToolUse(Write\|Edit): query janela aberta com `session_id` matching; se `file_path` ∈ `pipeline_file_set` ou subprojeto pertence a `subprojects`, emit `pipeline.amend_activity` e atualiza `last_activity_at`
- [ ] PostToolUse(Bash): se comando casa regex `cargo (build|test|check)|pnpm (build|test)|npm (test|run build)|tsc` E exit_code 0, atualiza `build_verde_at`
- [ ] UserPromptSubmit: se há janela aberta com session matching, emit `pipeline.amend_intent` com `prompt_text` literal preservado
- [ ] Drift detection: edit em arquivo (a) fora `pipeline_file_set` E (b) subprojeto fora `subprojects` → adiciona path único ao `drift_unrelated_paths`; se `len(drift_unrelated_paths) ≥ amend.drift_threshold` (default 3) E `drift_emitted=0`, emit `pipeline.amend_drift` + retorna `Decision::Allow` com injection contendo warning em PT/EN herdado da spec; set `drift_emitted=1`
- [ ] Registrar module em `Registry::new()` em `apps/rt/src/dispatch.rs` (append-only, NUNCA modificar dispatcher)
- [ ] Estender `prompt_gate` para fechar janelas abertas antes de iniciar nova `/mustard:feature` ou `/mustard:bugfix`
- [ ] Tests: AC-3, AC-4, AC-5, AC-6, AC-8, AC-9, AC-10
- [ ] `cargo build -p mustard-rt && cargo test -p mustard-rt`

### rt Agent (Wave 3) — Auto-amend writer + SessionEnd

- [ ] Criar subcommand `mustard-rt run amend-finalize --session-id {id}` em `apps/rt/src/run/amend_finalize.rs`
- [ ] amend-finalize: para cada janela com `session_id` matching e `status != resolved/drift`:
...
- [ ] Estender `apps/rt/src/hooks/session_cleanup.rs` para chamar `amend-finalize` ANTES de qualquer cleanup
- [ ] Garantir ordenação no `Registry::new()`: amend_capture (SessionEnd) precede session_cleanup (SessionEnd)
- [ ] Tests: AC-11, AC-12, AC-13, AC-14
- [ ] `cargo build -p mustard-rt && cargo test -p mustard-rt`

### dashboard Agent (Wave 4) — Timeline + telemetry séries (parallel-safe com Wave 3)

- [ ] Estender `apps/dashboard/src-tauri/src/queries.rs` com 4 funções:
...
- [ ] Expor via Tauri `#[command]`
- [ ] Estender `Activity.tsx`: sub-eventos `pipeline.amend_*` aparecem agrupados dentro do card da spec original (não card separado); ícone distinto pra amend
- [ ] Estender `Telemetry.tsx`: 4 séries novas como cards/charts na aba Atividade
- [ ] Tests: AC-15, AC-16
- [ ] `pnpm --filter mustard-dashboard build && pnpm --filter mustard-dashboard test`
