...
### Status: completed | Phase: CLOSE | Scope: full | Wave: 2a/3
...
- [ ] **git/SKILL.md split (588 → ≤200):**
  - Read full original.
  - Extract autonomous sections to:
    - `references/git-flow.md` → tabelas de comandos, fluxo dev_rubens/dev/main, config `mustard.json`
    - `references/submodule-rules.md` → detecção/escopo de submódulos, rules de commit por subproject
    - `references/merge-protocol.md` → ff-only protocol, `/git merge main` flow, tratamento de conflitos
  - SKILL.md final: YAML frontmatter + Trigger + Description + 1-line por ação com pointer `→ references/X.md` quando o detalhe importa.
- [ ] **scan/SKILL.md split (450 → ≤200):**
  - Read full original (inclui mudanças unstaged: EVIDENCE RULE + Validate Skills step).
  - Extract:
    - `references/scan-protocol.md` → execution rule, agent dispatch sequence, `--force` handling, read-before-write, EXECUTION RULE "NO CONFIRMATION PROMPTS"
    - `references/evidence-rules.md` → EVIDENCE RULE 1-5 + seção Validate Skills
  - SKILL.md final: trigger + fases resumidas (1 linha cada) + refs.
- [ ] **templates/CLAUDE.md compactação (206 → ≤200):**
  - Read full.
  - Verificar se `templates/pipeline-config.md` existe (se não, verificar `templates/.claude/pipeline-config.md`).
  - Mover `## Cost Optimization Hooks` + `## Enforcement Hooks` tables + `## Shared Memory Architecture` (incluindo subseções "Truth source", "Persistent projections", "How agents read context", "Removed (Wave 4)", "Log rotation", "On-Demand Memory Queries") para `pipeline-config.md`.
...
- [ ] Copy `templates/commands/mustard/{git,scan}/SKILL.md` → `.claude/commands/mustard/{git,scan}/SKILL.md`
- [ ] Copy `templates/commands/mustard/{git,scan}/references/` → `.claude/commands/mustard/{git,scan}/references/`
- [ ] Verify each `.claude/` file ≤200 lines.

### General Agent (Wave 2a.3 — validation, sequential)

- [ ] Run `node templates/scripts/skill-validate.js --lines --json` → no `tier === "block"` for git/scan skills.
- [ ] **Content preservation grep** (each must find match in SKILL.md OR references/):
...
- [x] AC-3: Key tokens preserved — Command: `bash -c 'grep -rq "mustard.json" templates/commands/mustard/git/ && grep -rq "ff-only" templates/commands/mustard/git/ && grep -rq "EVIDENCE RULE" templates/commands/mustard/scan/ && grep -rq "MUSTARD_BASH_REDIRECT_MODE" templates/'`
- [x] AC-4: All new references/ files non-empty — Command: `node -e "const fs=require('fs');const path=require('path');const dirs=['templates/commands/mustard/git/references','templates/commands/mustard/scan/references'];for(const d of dirs){const files=fs.readdirSync(d).filter(f=>f.endsWith('.md'));if(!files.length){console.error('EMPTY DIR:',d);process.exit(1)}for(const f of files){if(!fs.statSync(path.join(d,f)).size){console.error('EMPTY:',f);process.exit(1)}}}"`
...
- [x] AC-6: Hook tests still pass — Command: `node -e "const{execSync}=require('child_process');try{execSync('node --test templates/hooks/__tests__/hooks.test.js templates/hooks/__tests__/size-gates.test.js',{stdio:'pipe',timeout:120000})}catch(e){process.exit(1)}"`
