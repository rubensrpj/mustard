...
- [x] ANALYZE: enumerar todo leitor `*_from_jsonl` em `telemetry.rs`/`lib.rs` e, para cada um, confirmar se já existe equivalente SQLite em `db.rs`. Listar os gaps de paridade.
- [x] Decidir (PLAN): o dashboard reusa `mustard-core::SqliteEventStore` (passa a depender da crate; alinha o pin `rusqlite 0.31` automaticamente) ou mantém o `db.rs` próprio. Registrar a decisão.
- [x] Fechar os gaps de paridade: implementar em `db.rs` (ou via `SqliteEventStore`) os leitores que hoje só existem em JSONL — agent activity, tool breakdown, cut-off de sessão, recent-events tail.
- [x] Remover os leitores `*_from_jsonl` e a lógica "JSONL-first / SQLite-fallback"; o SQLite passa a ser o único caminho. Corrigir os comentários factualmente errados.
- [x] `watcher.rs`: repontar a observação de mudança de `events.jsonl` para `mustard.db` (WAL — observar o `.db` e/ou o `-wal`).
- [x] `discovery.rs`: detecção de projeto-mustard deixa de depender de `events.jsonl`.
- [x] `cargo build -p mustard-dashboard` (ou o nome da crate `src-tauri`) verde; testes da crate verdes.

### Impl Agent (Wave 2) — depreciar o auto-update npm da CLI

- [x] `auto_update.rs`: o subcomando `auto-update` deixa de chamar `npm`; imprime que a CLI agora é binário Rust e indica o canal de atualização correto (ex.: `cargo install --git …` / GitHub releases). `--check-only` reporta a depreciação; nunca executa `npm install`.
- [x] `npm.rs`: remover `get_latest_version`/`update_global`/`PACKAGE_NAME` se ficarem sem uso após a depreciação; senão, marcar `#[deprecated]` com nota.
- [x] `cli.rs`: manter o subcomando `AutoUpdate` reconhecível (não quebrar quem o digita) mas com o novo comportamento.
- [x] `cargo build -p mustard-cli` + `cargo test -p mustard-cli` verdes.
...
- [x] AC-2: Testes do workspace verdes — Command: `bash -c 'cargo test --workspace'`
- [x] AC-3: Nenhum leitor de `events.jsonl` resta no backend do dashboard — Command: `bash -c '! grep -rln "events.jsonl" apps/dashboard/src-tauri/src'`
- [x] AC-4: O auto-update da CLI não invoca npm — Command: `bash -c '! grep -rln "npm install" apps/cli/src'`
