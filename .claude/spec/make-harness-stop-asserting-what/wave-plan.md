...
## Acceptance Criteria
- AC-1 — when the criterion proof is taken again after a wave's work has landed, then a criterion that still comes back red is reported as unproven instead of clearing on its earlier failure alone. Command: `cargo test -p mustard-rt ac_proof_requires_green_after` Expect: `ok\. [1-9][0-9]* passed`
