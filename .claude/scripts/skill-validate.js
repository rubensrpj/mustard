...
function collectSkillDirs() {
  ...
  if (subs.length === 0) {
    try {
      ...
      process.stderr.write(`[skill-validate] fallback discovery failed: ${err.message}\n`);
    }
  }
...
/** Resolve a possibly-relative path against project root, then check existence. */
function pathExistsUnderRoot(p) {
  if (!p) return false;
  try {
...
/**
 * Run factual checks on a single SKILL.md. Returns list of violations.
 * Returns [] (with gated=true) when skill lacks the `<!-- mustard:generated -->` header.
 *
 * @param {string} skillPath - absolute path to SKILL.md
 * @param {{ suffixes: Set<string> }} clusterIndex
 * @returns {{ gated: boolean, violations: Array<{ code: string, detail: string }> }}
 */
function factualCheckSkill(skillPath, clusterIndex) {
  const violations = [];
  let content;
  try { content = fs.readFileSync(skillPath, 'utf-8'); } catch (err) {
    // Fail-open: unreadable file is not a factual violation.
    process.stderr.write(`[skill-validate] unreadable: ${skillPath}: ${err.message}\n`);
    return { gated: true, violations };
  }
  ...
  // 2. Frequency gate — skill name must contain some cluster suffix as a token.
  ...
  if (name) {
    ...
    const parts = rawParts[rawParts.length - 1] === 'pattern'
      ? rawParts.slice(0, -1)
      : rawParts;
    ...
    if (parts.length >= 1) {
      ...
      // 2a. Try every contiguous sub-sequence of 1..N tokens (covers both single-word
      ...
      // 2b. Fallback: full original name is also acceptable (no subproject prefix in registry).
  ...
  // 3. Existence gate — sample paths must exist.
  const samples = extractSamplePaths(content);
  for (const p of samples) {
    if (!pathExistsUnderRoot(p)) {
      violations.push({ code: 'STALE_SAMPLE', detail: p });
    }
  }

  // 4. No-code gate — no fenced blocks allowed in SKILL.md body.
  const fenceCount = countFencedBlocks(content);
  if (fenceCount > 0) {
    violations.push({ code: 'CODE_IN_BODY', detail: `${fenceCount} fenced code block(s)` });
  }

  // 5. References gate — Source: `{path}` entries in references/examples.md must exist.
  const refsFile = path.join(path.dirname(skillPath), 'references', 'examples.md');
  if (fs.existsSync(refsFile)) {
    let refsContent;
    try { refsContent = fs.readFileSync(refsFile, 'utf-8'); } catch (err) {
      process.stderr.write(`[skill-validate] unreadable references: ${refsFile}: ${err.message}\n`);
      refsContent = null;
    }
    if (refsContent) {
...
function runFactualMode() {
  ...
  try { clusterIndex = loadClusterIndex(); } catch (err) {
    process.stderr.write(`[skill-validate] cluster index load failed: ${err.message}\n`);
  }
  ...
  for (const { dir, label } of locations) {
    let files = [];
    try { files = collectSkills(dir); } catch (err) {
      process.stderr.write(`[skill-validate] collect failed for ${dir}: ${err.message}\n`);
      continue;
    }
    for (const file of files) {
      total++;
      let result;
      try { result = factualCheckSkill(file, clusterIndex); } catch (err) {
        process.stderr.write(`[skill-validate] check failed for ${file}: ${err.message}\n`);
        continue;
      }
      ...
      report.push({ skill: skillName, file: rel, violations: result.violations.length });
    }
  }
  ...
  if (JSON_OUT) {
    ...
    if (violationsAll.length === 0) {
      ...
      console.log(`skill-validate (factual): ${violationsAll.length} violation(s) across ${total} skill(s). mode=${FACTUAL_MODE}`);
      // Group by skill for readability.
      const bySkill = new Map();
      for (const v of violationsAll) {
        if (!bySkill.has(v.file)) bySkill.set(v.file, []);
        bySkill.get(v.file).push(v);
      }
      for (const [file, vs] of bySkill) {
        console.log(`  ${file}`);
        for (const v of vs) {
  ...
  // strict mode: exit 1 if any violation surfaced
...
function runLinesMode() {
  if (LINES_MODE === 'off') {
    if (JSON_OUT) {
      process.stdout.write(JSON.stringify({ mode: 'off', total: 0, results: [] }, null, 2) + '\n');
    } else {
  ...
  if (JSON_OUT) {
    ...
    if (notable.length === 0) {
      console.log(`skill-validate (lines): ${results.length} skill(s) scanned, all within thresholds. mode=${LINES_MODE}`);
    } else {
      console.log(`skill-validate (lines): ${notable.length} skill(s) above threshold. mode=${LINES_MODE}`);
      for (const r of notable) {
        console.log(`  [LINES] ${r.file}: ${r.lineCount} lines — tier=${r.tier}`);
      }
    }
  }
